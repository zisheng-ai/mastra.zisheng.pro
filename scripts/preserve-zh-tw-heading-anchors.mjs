...
if (unknownArguments.length > 0) {
  ...
  console.error('Usage: node scripts/preserve-zh-tw-heading-anchors.mjs [--check]')
  process.exit(2)
}
...
const contentRoots = [
  ...
  },
  {
    ...
    target: 'src/learn/content/zh-TW',
    recursive: false,
  },
]
...
for (const contentRoot of contentRoots) {
  ...
  for (const relativeFile of sourceRelativeFiles) {
    ...
    rootPages += 1
    rootHeadings += sourceHeadings.filter(heading => heading.depth > 1).length

    if (sourceHeadings.length !== targetHeadings.length) {
      errors.push(
        `${contentRoot.name}/${relativeFile}: heading count differs (${sourceHeadings.length} English, ${targetHeadings.length} zh-TW)`,
      )
      continue
    }
    ...
    for (let index = 0; index < sourceHeadings.length; index += 1) {
      ...
      if (sourceHeading.depth !== targetHeading.depth) {
        errors.push(
          `${contentRoot.name}/${relativeFile}: heading ${index + 1} depth differs (${sourceHeading.depth} English, ${targetHeading.depth} zh-TW)`,
        )
        pageFailed = true
        continue
      }
      ...
      // Docusaurus does not create autolink anchors for H1 headings. Keeping
      // their localized IDs avoids polluting generated page metadata.
    ...
    if (!pageFailed && addedAnchors + normalizedAnchors + removedH1Anchors > 0) {
...
if (checkOnly) {
  ...
  console.log('\nAll zh-TW headings preserve their English anchors.')
} else if (plans.length === 0) {
  console.log('\nNo changes needed; all zh-TW headings already preserve their English anchors.')
} else {
  await Promise.all(plans.map(plan => fs.writeFile(plan.file, plan.content)))
  console.log(`\nUpdated ${plans.length} zh-TW file(s).`)
}
