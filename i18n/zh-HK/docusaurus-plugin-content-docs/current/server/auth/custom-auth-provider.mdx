---
title: "自訂身份 Provider | 身份驗證"
description: "為專門的身份系統建立自訂身份 Provider"
packages:
  - "@mastra/core"
  - "@mastra/auth"
---
...
自訂身份 Provider 讓你為內置 Provider 未涵蓋的身份系統實作身份驗證。擴充 `MastraAuthProvider` 基礎類別，即可與任何身份驗證系統整合。

## 概覽

身份 Provider 負責處理傳入請求的身份驗證及授權：
...
驗證傳入的 token；如有效則傳回用戶物件，身份驗證失敗則傳回 `null`。
...
| `request` | `HonoRequest` | 傳入的請求物件（可存取 header、cookie 等） |

**傳回值**：身份驗證成功時傳回用戶物件，失敗時則傳回 `null`。

系統會自動從 `Authorization: Bearer <token>` header 擷取 token。如要存取其他 header 或 cookie，請使用 `request` 參數。
...
| 參數 | 類型 | 說明 |
| - | - | - |
| `user` | `TUser` | `authenticateToken` 傳回的用戶物件 |
| `request` | `HonoRequest` | 傳入的請求物件 |

**傳回值**：傳回 `true` 允許存取，傳回 `false` 則拒絕存取（傳回 403 Forbidden）。
...
| 選項 | 類型 | 說明 |
| - | - | - |
| `name` | `string` | 用於記錄日誌／除錯的 Provider 名稱 |
| `authorizeUser` | `(user, request) => Promise<boolean> \| boolean` | 自訂授權函式 |
| `protected` | `(RegExp \| string \| [string, Methods \| Methods[]])[]` | 需要身份驗證的路徑 |
| `public` | `(RegExp \| string \| [string, Methods \| Methods[]])[]` | 略過身份驗證的路徑 |

### 路徑 pattern

使用 pattern matching 配置哪些路徑需要身份驗證：
...
在 Mastra instance 註冊自訂身份 Provider：
...
`@mastra/auth` 依賴套件提供適用於常見 token 驗證 pattern 的工具：
...
Mastra 包含以下身份 Provider，作為參考實作：

- **MastraJwtAuth**：使用 HMAC secret 進行簡單 JWT 驗證（`@mastra/auth`）
- **MastraAuthClerk**：Clerk 身份驗證（`@mastra/auth-clerk`）
- **MastraAuthAuth0**：Auth0 身份驗證（`@mastra/auth-auth0`）
- **MastraAuthSupabase**：Supabase 身份驗證（`@mastra/auth-supabase`）
- **MastraAuthFirebase**：Firebase 身份驗證（`@mastra/auth-firebase`）
- **MastraAuthWorkOS**：WorkOS 身份驗證（`@mastra/auth-workos`）
- **MastraAuthBetterAuth**：Better Auth 整合（`@mastra/auth-better-auth`）
- **SimpleAuth**：適用於開發的 token 對應用戶方式（`@mastra/core/server`）

請參閱[源碼](https://github.com/mastra-ai/mastra/tree/main/auth)，了解實作詳情。

## 相關內容

- [身份驗證概覽](/docs/server/auth)：身份驗證概念及配置
- [自訂 API 路由](/docs/server/custom-api-routes)：控制自訂 endpoint 的身份驗證
