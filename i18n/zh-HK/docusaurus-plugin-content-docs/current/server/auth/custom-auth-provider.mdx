---
title: "自訂驗證 Provider | 驗證"
description: "為專門的身份系統建立自訂驗證 Provider"
packages:
  - "@mastra/core"
  - "@mastra/auth"
---

# 自訂驗證 Provider {/* #custom-auth-provider */}
...
## 建立自訂驗證 Provider {/* #creating-a-custom-auth-provider */}
...
| `request` | `HonoRequest` | 傳入的請求物件（存取 header、cookie 等） |
...
| 選項 | 類型 | 說明 |
| - | - | - |
| `name` | `string` | 用於記錄／除錯的 Provider 名稱 |
| `authorizeUser` | `(user, request) => Promise<boolean> \| boolean` | 自訂授權函數 |
...
## 使用你的驗證 Provider {/* #using-your-auth-provider */}
...
### 以角色為基礎的授權 {/* #role-based-authorization */}
...
## 測試自訂驗證 Provider {/* #testing-custom-auth-providers */}
...
## 內置 Provider {/* #built-in-providers */}
...
如需實作詳情，請參閱[原始碼](https://github.com/mastra-ai/mastra/tree/main/auth)。

## 相關內容 {/* #related */}

- [驗證概覽](/docs/server/auth)：驗證概念和配置
- [自訂 API 路由](/docs/server/custom-api-routes)：控制自訂 endpoint 的驗證
