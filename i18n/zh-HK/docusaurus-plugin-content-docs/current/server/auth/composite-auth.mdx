---
title: "Composite Auth | 身份驗證"
description: "CompositeAuth class 的文件，說明如何將多個身份驗證 Provider 合併為單一身份驗證 handler。"
packages:
  - "@mastra/core"
---
...
`CompositeAuth` class 可讓你將多個身份驗證 Provider 合併為單一身份驗證 handler。它會依次嘗試每個 Provider，直至其中一個成功。

## 使用情境

- 同時支援 API 金鑰及 OAuth token
- 在身份驗證 Provider 之間遷移，而不影響現有客戶端
- 允許使用多個 identity Provider（例如網絡使用 Clerk、整合使用 API 金鑰）
- 逐步推出新的身份驗證方法

## 安裝

CompositeAuth 已包含在 `@mastra/core` 中，無需額外依賴套件。
...
將 SimpleAuth（用於 API 金鑰）與 Clerk（用於使用者 session）結合：
...
1. 依次嘗試每個 Provider 的 `authenticateToken()` method
1. 傳回首個驗證成功的 Provider 所提供的使用者
1. 如果所有 Provider 均失敗，則傳回 `null`（401 Unauthorized）

進行授權時，它會呼叫每個 Provider 的 `authorizeUser()` method，直至其中一個傳回 `true`。
...
Provider 次序會影響效能，因此請將最常用的身份驗證方法放在最前：
...
CompositeAuth 會在不發出提示的情況下捕捉個別 Provider 的錯誤，然後繼續嘗試下一個。這可避免單一 Provider 失敗時阻礙身份驗證：
...
如要偵錯身份驗證問題，請在自訂 Provider 加入 logging，或檢查個別 Provider 的設定。

## 限制

- 所有 Provider 共用 `Authorization` header 中的相同 token
- 不同 Provider 的使用者 type 可能有別（如有需要，可使用 discriminated union）
- 沒有內置方法可識別驗證請求的是哪個 Provider

### 處理不同的使用者 type

當 Provider 傳回不同的使用者 type 時，請使用 discriminated union：
