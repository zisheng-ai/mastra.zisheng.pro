...
## 所有參與者共享一個 `resourceId` {/* #share-one-resourceid-across-all-participants */}
...
## 為每則用戶訊息標記發言者身分 {/* #tag-each-user-message-with-the-speakers-identity */}
...
## 與不同記憶層配合使用 {/* #combining-with-memory-layers */}
...
### 只使用訊息歷史記錄 {/* #message-history-alone */}
...
### 使用 Observational Memory（建議） {/* #with-observational-memory-recommended */}
