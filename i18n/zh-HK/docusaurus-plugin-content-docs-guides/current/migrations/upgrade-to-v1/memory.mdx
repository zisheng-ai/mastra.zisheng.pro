...
### 語義回憶及最近訊息的預設設定 {/* #default-settings-for-semantic-recall-and-last-messages */}
...
### 對話串標題產生選項的位置 {/* #thread-title-generation-location */}
...
### 最佳化語義回憶的預設設定 {/* #semantic-recall-default-settings-optimization */}
...
### `MastraMessageV2` 類型重新命名為 `MastraDBMessage` {/* #mastramessagev2-type-renamed-to-mastradbmessage */}
...
### Working memory `text-stream` 模式 {/* #working-memory-text-stream-mode */}
...
### 從 Memory 方法移除 `format` 參數 {/* #format-parameter-from-memory-methods */}
...
### `MastraMessageV3` 類型 {/* #mastramessagev3-type */}
...
### 從 Memory 建構函式移除 `processors` 設定 {/* #processors-config-from-memory-constructor */}
