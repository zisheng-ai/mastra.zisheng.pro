...
# 從 `.network()` 遷移至 supervisor Agent {/* #migrate-from-network-to-supervisor-agents */}
...
## 以 `.stream()` 或 `.generate()` 取代 `.network()` {/* #replace-network-with-stream-or-generate */}
...
## 撰寫清晰的 supervisor 指示 {/* #write-clear-supervisor-instructions */}
...
## 為 subagent 加入描述 {/* #add-descriptions-to-subagents */}
...
## 加入委派 hook {/* #add-delegation-hooks */}
...
## 加入迭代監察 {/* #add-iteration-monitoring */}
...
## 加入工作完成度評分 {/* #add-task-completion-scoring */}
