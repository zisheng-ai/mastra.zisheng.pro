...
<PropertiesTable
  ...
  },
  {
    name: 'errorPatterns',
    type: 'RegExp[]',
    description: '用於匹配 Provider API 錯誤訊息及回應內容的模式。實作 fix 的反應式規則必須提供此項。',
    isOptional: true,
  },
  {
    name: 'fix',
    type: '(messages: MastraDBMessage[]) => boolean',
    description: '匹配 API 錯誤後修改已持久保存資料庫訊息的反應式修正。如規則已變更訊息且應重試請求，請返回 true。',
    isOptional: true,
  },
  {
    name: 'applyToPrompt',
    type: '(args: { prompt: LanguageModelV2Prompt; model: unknown }) => LanguageModelV2Prompt | undefined',
    description: '為目前 Provider 呼叫重寫傳出 prompt 的先發修正。如無需變更 prompt，請返回 undefined。',
    isOptional: true,
  },
]}
/>
