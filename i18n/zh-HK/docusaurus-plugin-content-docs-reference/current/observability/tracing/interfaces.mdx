...
此映射定義建立或處理 span 時，各個 span 類型所使用的屬性介面。
...
事件 callback payload 使用可觀測性事件匯流排 envelope：
`TracingEvent` 透過 `exportedSpan` 傳遞 span 生命週期事件；`LogEvent`
在 `log` 中封裝 `ExportedLog`；`MetricEvent` 在 `metric` 中封裝
`ExportedMetric`；`ScoreEvent` 在 `score` 中封裝 `ExportedScore`；而
`FeedbackEvent` 則在 `feedback` 中封裝 `ExportedFeedback`。有關這些 callback
在 Mastra 平台 exporter 中的行為，請參閱 [MastraPlatformExporter](/reference/observability/tracing/exporters/mastra-platform-exporter)。

與 `LogEvent`、`MetricEvent` 及 `FeedbackEvent` 一樣，`ScoreEvent` 是一個
封裝有界 payload 的可觀測性匯流排 envelope。對 score 而言，該 payload
是 `ExportedScore`。

### `ScoreEvent`

score 事件會透過 `onScoreEvent` 傳送至 exporter。此事件是一個小型
envelope，當中包含訊號類型及 score payload：
...
<PropertiesTable
  props={[
  {
    name: 'type',
    type: "'score'",
    description: '將此可觀測性匯流排事件識別為 score 訊號。',
    required: true,
  },
  {
    name: 'score',
    type: 'ExportedScore',
    description: 'exporter 接收的 score payload。',
    required: true,
  },
]}
/>
...
`ExportedScore` 是 exporter 在 `ScoreEvent.score` 內接收的有界 payload。
它包含 score 身分、目標 Trace 或 span 錨點、scorer 詳情、數值、選用說明及
關聯 metadata。
...
<PropertiesTable
  props={[
  {
    name: 'scoreId',
    type: 'string',
    description: '此 score 事件的唯一識別碼，於發出時產生。',
    required: true,
  },
  {
    name: 'timestamp',
    type: 'Date',
    description:
      '記錄 score 的時間。exporter 會收到 Date 物件，該物件透過 JSON 序列化為 ISO 8601 字串。',
    required: true,
  },
  {
    name: 'traceId',
    type: 'string',
    description: '如適用，用作已評分目標錨點的 Trace。',
  },
  {
    name: 'spanId',
    type: 'string',
    description: '當 score 涉及特定 span 時使用的 span 錨點。',
  },
  {
    name: 'scorerId',
    type: 'string',
  ...
  },
  {
    name: 'source',
    type: 'string',
    description: '已棄用的相容性欄位。新的 exporter 請使用 scoreSource。',
  },
  {
    name: 'scoreSource',
    type: 'string',
    description:
      '識別 score 產生方式的自由格式字串。常見值包括 manual、automated 及 experiment。',
  },
  {
    name: 'score',
    type: 'number',
  ...
  },
  {
    name: 'scoreTraceId',
    type: 'string',
    description: '評分執行本身的 Trace ID，有助除錯 score 產生過程。',
  },
  {
    name: 'targetEntityType',
    type: 'EntityType',
    description: '如已知，scorer 所評估的 Mastra 可觀測性實體類型。',
  },
  {
    name: 'correlationContext',
    type: 'CorrelationContext',
    description: '用於實體階層、執行、部署及實驗 metadata 的標準關聯 context。',
  },
  {
    name: 'metadata',
    type: 'Record<string, unknown>',
    description:
      '從已評分 span 或 Trace 繼承的使用者定義 metadata，並與 score 專用 metadata 合併。如 key 發生衝突，score 專用 key 會覆寫繼承的 key。',
  },
]}
/>

`traceId` 及 `spanId` 識別正在評分的 Trace 或 span。當該 scorer 有被 tracing
時，`scoreTraceId` 會識別評分執行本身的 Trace。對新的 exporter，應優先使用
`scoreSource`，而非已棄用的 `source` 欄位；亦應優先使用
`correlationContext.experimentId`，而非已棄用的頂層 `experimentId` 欄位。
...
`CorrelationContext` 是附加至可觀測性訊號的共用 context snapshot。它可攜帶
實體階層欄位、使用者或機構識別碼、執行、session、thread、request、environment、
source、service name、experiment 及 tag。對已評分目標，應優先使用
`ExportedScore` 的頂層 `traceId` 及 `spanId` 欄位。

### `ObservabilityDropEvent`

exporter pipeline 捨棄可觀測性事件時發出的結構化事件。
...
提供予模型的單一 Tool 序列化定義，附加至 `MODEL_GENERATION` span，讓可觀測性 exporter 可呈現 Tool schema。
...
在 Workflow 及 Agent 執行期間傳遞的 Tracing context。
...
開始新的 Agent 或 Workflow 執行時傳入的選項。
...
建立 Workflow 或 Agent 時套用的 policy-level tracing 設定。
...
選擇 span 要使用哪個可觀測性 instance 的函數。
...
使用自訂 tracing 設定 selector 時傳入的選項。
...
在 Workflow 或 Agent 執行中將不同類型 span 設為內部 span 的 bitwise 選項。
...
- [加入自訂 metadata](/docs/observability/tracing/overview#adding-custom-metadata)：豐富 Trace 資料
