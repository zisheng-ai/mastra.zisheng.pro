...
`createInngestAgent()` 以由 [Inngest](https://www.inngest.com/docs) 驅動的持久執行來包裝現有的 [`Agent`](/reference/agents/agent)。它與 [`createDurableAgent()`](/reference/agents/durable-agent#createdurableagentoptions) 一樣，透過 [PubSub](/docs/server/pubsub) 串流傳送事件，並支援可恢復串流；但 Agent 迴圈會在 Inngest 的執行引擎上執行，而非在處理程序內執行。當一次執行必須不受處理程序重新啟動影響，或需要在分散式環境中執行時，請使用此函式。
