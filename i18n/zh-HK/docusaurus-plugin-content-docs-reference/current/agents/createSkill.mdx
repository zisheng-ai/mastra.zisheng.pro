---
title: "參考：createSkill() | Skills"
description: "createSkill() 的 API 參考；這個工廠函式毋須檔案系統即可建立內嵌 Agent Skill。"
packages:
  - "@mastra/core"
---

# `createSkill()` {/* #createskill */}

建立可直接傳送至 Agent `skills` 設定的內嵌 Skill，毋須檔案系統或 Workspace。

傳回的物件會實作 `Skill` 介面，並可在任何支援透過檔案系統找到 Skill 的地方使用。

## 使用範例 {/* #usage-example */}

```typescript title="src/mastra/skills.ts"
import { createSkill } from '@mastra/core/skills'

const reviewSkill = createSkill({
  name: 'code-review',
  description: 'Use when reviewing code changes.',
  instructions: `
    When reviewing code:
    1. Check for correctness
    2. Check for style consistency
    3. Look for potential bugs
  `,
  references: {
    'checklist.md': '# Review Checklist\n- No unused imports\n- Error handling present',
  },
})
```

## 參數 {/* #parameters */}

<PropertiesTable
  content={[
  {
    name: 'name',
    type: 'string',
    description:
      '獨有的 Skill 名稱。Agent 啟用 Skill 時會使用此名稱作為識別碼。必須是有效的 slug（小寫，允許使用連字號）。',
  },
  {
    name: 'description',
    type: 'string',
    description: '在系統訊息中向 Agent 顯示的簡短說明，協助 Agent 決定何時使用此 Skill。',
  },
  {
    name: 'instructions',
    type: 'string',
    description: '採用 Markdown 格式的完整 Skill 指示。Agent 啟用 Skill 時，這些指示會載入對話。',
  },
  {
    name: 'references',
    type: 'Record<string, string>',
    isOptional: true,
    description: 'Agent 可使用 `skill_read` Tool 讀取的支援文件。鍵為檔案名稱，值為檔案內容。',
  },
  {
    name: 'license',
    type: 'string',
    isOptional: true,
    description: 'Skill 的 SPDX 授權條款識別碼。',
  },
  {
    name: 'compatibility',
    type: 'unknown',
    isOptional: true,
    description: '兼容性要求。Agent Skills 規格容許彈性設定，可以是字串陣列、物件，或工具預期的任何結構。',
  },
  {
    name: 'user-invocable',
    type: 'boolean',
    isOptional: true,
    description: '終端用戶能否直接叫用此 Skill。預設為 `undefined`（由 Agent 決定）。',
  },
  {
    name: 'metadata',
    type: 'Record<string, unknown>',
    isOptional: true,
    description: '附加至 Skill 的任意中繼資料。',
  },
]}
/>

## 傳回值 {/* #return-value */}

傳回實作 `Skill` 介面的 `InlineSkill` 物件：

```typescript
interface InlineSkill extends Skill {
  __inline: true
  __referenceContents: Record<string, string>
}
```

此 Skill 會獲派合成路徑 `inline/<name>`（例如 `inline/code-review`）。

## 驗證 {/* #validation */}

`createSkill()` 使用與檔案系統 Skill 相同的規則驗證輸入：

- `name` 為必填，而且必須是有效的 slug
- `description` 為必填
- `instructions` 為必填，而且不得為空白

如果驗證失敗，便會擲回包含無效欄位詳細資料的 `Error`。

```typescript
// Throws: Invalid skill "": name is required
createSkill({ name: '', description: 'test', instructions: 'test' })
```

## 相關內容 {/* #related */}

- [Agent Skill](/docs/agents/skills)
- [Workspace Skill](/docs/workspace/skills)
- [`.getSkill()` 參考](/reference/agents/getSkill)
- [`.listSkills()` 參考](/reference/agents/listSkills)
