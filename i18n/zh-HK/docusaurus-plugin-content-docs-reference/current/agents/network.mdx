    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            isOptional: true,
            description: '網絡中止時觸發的回呼。接收一個事件，當中包含中止發生時正在執行的 primitive 類型及 ID。',
          },
          {
            name: 'memory',
            type: 'object',
          ...
          },
          {
            ...
            type: 'TracingContext',
            isOptional: true,
            description: '用於建立子 span 及加入 metadata 的 tracing context。使用 Mastra tracing 系統時會自動注入。',
            properties: [
              {
                type: 'TracingContext',
                parameters: [
                ...
                parameters: [
                  {
                    ...
                    type: 'boolean',
                    isOptional: true,
                    description: '啟用或停用輸入記錄。預設為啟用。為免記錄敏感資料，你可能需要停用輸入記錄。',
                  },
                ],
              },
              {
                parameters: [
                  {
                    ...
                    type: 'boolean',
                    isOptional: true,
                    description: '啟用或停用輸出記錄。預設為啟用。為免記錄敏感資料，你可能需要停用輸出記錄。',
                  },
                ],
              },
              {
          ...
          },
          {
            ...
            type: 'string',
            isOptional: true,
            description: '啟用 Tracing 時與此執行關聯的 Trace ID。可用於關聯記錄及除錯執行流程。',
          },
          {
            name: 'spanId',
            type: 'string',
            isOptional: true,
            description: '啟用 Tracing 時與此執行關聯的根 span ID。可用於 span 層級的查找及關聯。',
          },
          {
            name: 'onStepFinish',
            type: '(event: any) => Promise<void> | void',
            isOptional: true,
            description: '子 Agent 執行中每個 LLM 步驟完成後觸發的回呼。接收步驟詳情，包括完成原因及 token 用量。',
          },
          {
            name: 'onError',
...
<PropertiesTable
  content={[
  {
    ...
    description: '擴充 ReadableStream<NetworkChunkType> 並加入網絡特定屬性的自訂 stream',
  },
  {
    name: 'status',
  ...
  },
  {
    ...
    description: '產生結構化輸出期間的部分物件 stream，適合在產生部分結果時串流傳送。',
  },
]}
/>
...
### 串流傳送部分物件 {/* #streaming-partial-objects */}
