...
<PropertiesTable
        ...
        parameters: [
          {
            ...
            type: 'string',
            description:
              'Google Cloud API key。未提供時會使用 GOOGLE_API_KEY 環境變數。vertexAI 為 true 時不會使用。',
            isOptional: true,
          },
          {
            name: 'keyFilename',
            type: 'string',
            description:
              '服務帳戶 JSON key 檔案的路徑。未提供時會使用 GOOGLE_APPLICATION_CREDENTIALS 環境變數。',
            isOptional: true,
          },
          {
            name: 'credentials',
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      "為企業部署啟用 Vertex AI 模式。使用以專案為基礎的驗證，而非 API key。必須設定 'project'。",
    isOptional: true,
    defaultValue: 'false',
  },
  {
    ...
    type: 'string',
    description:
      'Google Cloud 專案 ID（vertexAI 為 true 時必須提供）。未提供時會使用 GOOGLE_CLOUD_PROJECT 環境變數。',
    isOptional: true,
  },
  {
    name: 'location',
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'string',
            description:
              "語音的語言代碼（例如 'en-US'）。預設使用從 speaker ID 得出的語言代碼，否則為 'en-US'。",
            isOptional: true,
          },
          {
            name: 'input',
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'string',
            description:
              "v2 recognizer 資源路徑。預設為 `projects/{project}/locations/global/recognizers/_`，當中的 `{project}` 會從建構函數的 `project` 選項、`GOOGLE_CLOUD_PROJECT` 或用戶端的預設專案解析。",
            isOptional: true,
          },
        ],
      },
...
傳回：`string | undefined`——專案 ID；如未設定則為 `undefined`
...
使用以 Google Cloud 專案為基礎的服務帳戶驗證。建議用於正式環境及企業部署。
...
1. Vertex AI 模式提供企業功能，包括 IAM 控制、稽核記錄及專案層級帳單。
