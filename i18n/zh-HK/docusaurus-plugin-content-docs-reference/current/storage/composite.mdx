...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      "預設儲存 adapter。未在 `domains` 明確指定的域，會使用此儲存的域作為後備。",
    isOptional: true,
  },
  {
    name: 'editor',
...
`ObservabilityStorageClickhouseVNext` 是目前的 observability 域實作。系統亦會匯出舊版 `ObservabilityStorageClickhouse` class，並繼續支援尚未遷移的專案。詳情請參閱 [ClickHouse 儲存參考](/reference/storage/clickhouse)。
