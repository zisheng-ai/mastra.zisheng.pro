...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    defaultValue: '1000',
    description: '每批刪除的資料列數目。每一批都是獨立交易，可限制大型資料表的鎖定時間及 WAL 增長。',
  },
]}
/>

### 符合保留資格的資料表 {/* #retention-eligible-tables */}
...
<PropertiesTable
  content={[
  {
    ...
    type: 'number',
    isOptional: true,
    description: '每個資料表在每次呼叫中的刪除批次上限。達到上限時，該資料表的結果會以 `done: false` 傳回。',
  },
  {
    name: 'maxRows',
    type: 'number',
    isOptional: true,
    description: '每個資料表在每次呼叫中的刪除資料列上限。達到上限時，該資料表的結果會以 `done: false` 傳回。',
  },
  {
    name: 'pauseMs',
    type: 'number',
  ...
  },
  {
    ...
    type: 'AbortSignal',
    isOptional: true,
    description: '協作式取消。批次循環會在批次之間檢查此項，並乾淨地停止，以 `done: false` 傳回部分結果。',
  },
  {
    name: 'retention',
    type: 'RetentionConfig',
...
## 按排程執行 prune {/* #running-prune-on-a-schedule */}
...
### 在 MongoDB 設定 TTL 索引 {/* #setting-up-ttl-indexes-on-mongodb */}
