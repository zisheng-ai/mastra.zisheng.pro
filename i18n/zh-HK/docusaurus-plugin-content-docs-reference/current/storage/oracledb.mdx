...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'oracledb.Pool',
    description:
      '現有的 Oracle 連線池。提供後，Mastra 會使用該連線池，但呼叫 `store.close()` 時不會將其關閉。',
    isOptional: true,
  },
  {
    name: 'poolManager',
    ...
    description:
      '共用的 Oracle 連線池管理器。用它讓 `OracleStore` 與 `OracleVector` 共用同一個 Oracle 連線池。',
    isOptional: true,
  },
  {
    name: 'schemaName',
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      '設為 true 時會停用自動 schema 初始化。若 schema 變更會在應用程式啟動前另行套用，請使用此選項。',
    isOptional: true,
    defaultValue: 'false',
  },
  {
    ...
    type: 'number',
    description:
      '儲存訊息時，每次 Oracle `executeMany` 呼叫傳送的訊息數目。操作仍只會在交易邊界提交一次。',
    isOptional: true,
    defaultValue: '200',
  },
  {
  ...
  },
  {
    ...
    description:
      '初始化期間要建立的自訂 Oracle 索引定義。索引會路由至擁有目標資料表的儲存網域。',
    isOptional: true,
  },
  {
