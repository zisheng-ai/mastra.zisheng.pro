...
### 配合 gate 及 threshold {/* #with-gates-and-thresholds */}
...
<PropertiesTable
  ...
  },
  {
    name: 'onItemComplete',
    type: 'function',
    description: '每個測試個案完成後呼叫的 callback 函數。接收項目、target 結果及 scorer 結果。',
    isOptional: true,
  },
]}
/>
...
## Agent scorer 設定 {/* #agent-scorer-configuration */}
...
<PropertiesTable
  content={[
  {
    name: 'agent',
    type: 'MastraScorer[]',
    description: '接收原始 Agent 輸出（MastraDBMessage[]）的 scorer。用於評估回應品質、內容等。',
    isOptional: true,
  },
  {
    name: 'trajectory',
...
## Workflow scorer 設定 {/* #workflow-scorer-configuration */}
...
<PropertiesTable
  ...
  },
  {
    name: 'gateResults',
    type: 'GateResult[]',
    description: '每個 gate 在所有資料項目上的平均結果。每個項目都有 `id`、`passed`（boolean）及 `score`（0–1）。',
    isOptional: true,
  },
  {
...
### Gate 及 verdict {/* #gates-and-verdict */}
...
### 配合 `targetOptions` 的 Agent {/* #agent-with-targetoptions */}
...
### 配合每個項目 `startOptions` 的 Workflow {/* #workflow-with-per-item-startoptions */}
...
### 每輪 assertion {/* #per-turn-assertions */}
