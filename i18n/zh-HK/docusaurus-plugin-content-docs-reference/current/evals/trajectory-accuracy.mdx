...
# Trajectory accuracy scorers {/* #trajectory-accuracy-scorers */}
...
## 擷取 trajectory {/* #trajectory-extraction */}

`runEvals` pipeline 會按 observability storage 是否已設定，採用兩種擷取策略：

### 以 Trace 為基礎的擷取方式（建議） {/* #trace-based-extraction-preferred */}
...
## Trajectory 類型 {/* #trajectory-types */}
...
<PropertiesTable
  ...
  },
  {
    ...
    description: '此步驟 children 的巢狀預期設定。評估此步驟的 children 時會覆寫上層設定。',
    isOptional: true,
  },
]}
/>
...
## 選擇 scorer {/* #choosing-between-scorers */}

### 適合使用以程式碼為基礎的 scorer 的情況 {/* #use-the-code-based-scorer-when */}
...
### 適合使用以 LLM 為基礎的 scorer 的情況 {/* #use-the-llm-based-scorer-when */}
...
## 以程式碼為基礎的 trajectory accuracy scorer {/* #code-based-trajectory-accuracy-scorer */}
...
### 預期 trajectory 的來源 {/* #expected-trajectory-sources */}
...
### 以程式碼為基礎的 scorer 結果 {/* #code-based-scorer-results */}
...
## 以程式碼為基礎的 scorer 範例 {/* #code-based-scorer-examples */}

### 採用嚴格排序的 Agent trajectory {/* #agent-trajectory-with-strict-ordering */}
...
### 採用寬鬆排序的 Agent trajectory {/* #agent-trajectory-with-relaxed-ordering */}
...
## 以 LLM 為基礎的 trajectory accuracy scorer {/* #llm-based-trajectory-accuracy-scorer */}
...
### 以 LLM 為基礎的 scorer 選項 {/* #llm-based-scorer-options */}
...
### 以 LLM 為基礎的 scorer 結果 {/* #llm-based-scorer-results */}
...
## 統一 trajectory scorer {/* #unified-trajectory-scorer */}
...
<PropertiesTable
  content={[
  {
    name: 'defaults',
    type: 'TrajectoryExpectation',
    description: '套用至所有 dataset 項目的預設預期設定。各項目的 expectedTrajectory 值會覆寫這些預設值。',
    isOptional: true,
    properties: [
      {
        name: 'steps',
      ...
      },
      {
        ...
        type: 'string[][]',
        description: '不得依次出現的 Tool 名稱序列。任何配對序列都會令分數變為 0。',
        isOptional: true,
      },
      {
...
### 統一 scorer 結果 {/* #unified-scorer-results */}
...
### 各項目的預期設定 {/* #per-item-expectations */}
...
### 範例：效率及黑名單 {/* #example-efficiency-and-blacklist */}
...
## 配合 `runEvals` 使用 trajectory scorer {/* #using-trajectory-scorers-with-runevals */}

Trajectory scorer 在 scorer 設定的 `trajectory` key 下配置。`runEvals` pipeline 會自動處理 trajectory 擷取。

### Agent trajectory 評估 {/* #agent-trajectory-evaluation */}
...
### Workflow trajectory 評估 {/* #workflow-trajectory-evaluation */}
