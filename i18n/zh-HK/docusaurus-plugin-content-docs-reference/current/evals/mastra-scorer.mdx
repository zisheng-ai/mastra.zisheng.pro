...
<PropertiesTable
  ...
  },
  {
    ...
    required: false,
    description:
      '在評分期間用作比較的可選預期或參考輸出。使用 runEvals 時會自動傳入。',
  },
]}
/>
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      '已完成評分器階段的輸出，以及曾嘗試的提示階段之 judge 執行證據。兩者皆無時會省略此屬性。',
  },
]}
/>

`result` 快照包含已完成階段的輸出及 judge 執行證據。例如，如果 `generateScore` 傳回 `0` 後 `generateReason` 失敗，`error.result.score` 會是 `0`，`generateScore` 執行的 `status` 為 `'success'`，而 `generateReason` 執行的 `status` 為 `'failed'`。整次執行仍視為失敗。
