...
<PropertiesTable
  ...
  },
  {
    name: 'ifIdle',
    type: 'ScheduleIfIdle',
    description: '目標 thread 處於閒置狀態時要執行的操作：`wake`、`persist` 或 `discard`。僅適用於使用 thread 的 schedules。',
    isOptional: true,
  },
  {
    name: 'status',
...
同步只會移除屬於目前程序中已註冊 Agent 的 rows，因此只包含部分 Agent 的程序絕不會刪除其他 Agent 的 schedules。當 Agent 從專案完全移除後，剩餘的 rows 會在下次觸發時清理，屆時 scheduler 會發現沒有可執行的 Agent。
