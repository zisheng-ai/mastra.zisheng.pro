...
- 對於**受管理**索引（建立時未提供 `collectionName`），`createIndex()` 已配置名為 `${collectionName}_search_index` 的_動態_全文索引（涵蓋所有字串欄位）。因此，只有需要**限制欄位的** mapping 或**自訂索引名稱**時，才需要使用 `createSearchIndex()`。
