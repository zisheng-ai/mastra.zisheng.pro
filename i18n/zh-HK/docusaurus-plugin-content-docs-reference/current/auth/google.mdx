...
# MastraAuthGoogle 和 MastraRBACGoogle 類別 {/* #mastraauthgoogle-and-mastrarbacgoogle-class */}
...
<PropertiesTable
  ...
  },
  {
    name: 'redirectUri',
    type: 'string',
    description: 'SSO 回呼的 OAuth 重新導向 URI。必須與 Google Cloud OAuth 用戶端中設定的重新導向 URI 相符。',
    isOptional: true,
  ...
  },
  {
    name: 'hostedDomain',
    type: 'string',
    description: '以 `hd` 傳遞給 Google 的託管網域登入提示。這只是一項提示，不會用於授權。',
    isOptional: true,
...
### 建構函數參數 {/* #constructor-parameters-1 */}
