...
請注意，如果你只需在 Mastra 應用程式內直接使用 Tools 或 Agents，未必需要建立 MCP 伺服器。此 API 專門用於向\_外部\_ MCP 用戶端公開 Mastra Tools 及 Agents。
...
<PropertiesTable
  content={[
  {
    ...
    type: 'string',
    isOptional: false,
    description: '伺服器的獨有識別碼。伺服器向 Mastra 註冊時會保留此 ID，並可用於透過 getMCPServerById() 擷取伺服器。',
  },
  {
    name: 'name',
    type: 'string',
  ...
  },
  {
    ...
    isOptional: true,
    description: '此伺服器版本的選擇性發佈日期（ISO 8601 字串）。如未提供，預設為實例化時間。',
  },
  {
    name: 'isLatest',
    type: 'boolean',
  ...
  },
  {
    ...
    isOptional: true,
    description: "伺服器以套件形式發佈時的選擇性 canonical 封裝格式（例如 'npm'、'docker'）。",
  },
  {
    name: 'packages',
    type: 'PackageInfo[]',
  ...
  },
  {
    ...
    isOptional: true,
    description: '定義伺服器如何處理 MCP Resources 的物件。詳情請參閱 Resource 處理章節。',
  },
  {
    name: 'prompts',
    type: 'MCPServerPrompts',
    isOptional: true,
    description: '定義伺服器如何處理 MCP Prompts 的物件。詳情請參閱 Prompt 處理章節。',
  },
  {
    name: 'appResources',
    type: 'AppResources',
...
## 將 Agents 公開為 Tools {/* #exposing-agents-as-tools */}
...
- **名稱衝突。** 如果在 `tools` 設定中明確定義的 Tool，與由 agent 衍生的 Tool 同名（例如名為 `ask_myAgentKey` 的 Tool 與鍵為 `myAgentKey` 的 agent 並存），則會\_優先採用明確定義的 Tool\_。發生此衝突時，該 agent 不會轉換為 Tool，並會記錄警告。
...
### 在 Tools 中存取 MCP Context {/* #accessing-mcp-context-in-tools */}
...
<PropertiesTable
  ...
  },
  {
    ...
    description: '初始化新 session 時呼叫的回呼函式，適合用於追蹤有效的 MCP sessions。',
    optional: true,
  },
  {
    name: 'enableJsonResponse',
    type: 'boolean',
    description: '如為 `true`，伺服器會傳回純 JSON 回應，而非使用 Server-Sent Events (SSE) 串流。預設為 `false`。',
    optional: true,
  },
  {
    name: 'eventStore',
    type: 'EventStore',
    description: '用於讓訊息可恢復的 event store。提供此選項後，用戶端可重新連接並恢復訊息串流。',
    optional: true,
  },
]}
/>
...
## Resource 處理 {/* #resource-handling */}

### 甚麼是 MCP Resources？ {/* #what-are-mcp-resources */}
...
### `MCPServerResources` 類型 {/* #mcpserverresources-type */}
...
### 通知用戶端 Resource 變更 {/* #notifying-clients-of-resource-changes */}
...
### 甚麼是 MCP Prompts？ {/* #what-are-mcp-prompts */}
...
### `MCPServerPrompts` 類型 {/* #mcpserverprompts-type */}
...
### 通知用戶端 Prompt 變更 {/* #notifying-clients-of-prompt-changes */}
...
### 甚麼是 Elicitation？ {/* #whats-elicitation */}
...
### Tool 執行簽署 {/* #tool-execution-signature */}
...
### 在 Tools 中使用 Elicitation {/* #using-elicitation-in-tools */}
...
### Elicitation 請求 Schema {/* #elicitation-request-schema */}
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description: '驗證 access token 的函式。如未提供，系統會接受 token 而不作驗證（不建議在正式環境使用）。',
  },
  {
    name: 'mcpPath',
    type: 'string',
...
### 為 FGA 映射驗證資料 {/* #map-auth-data-for-fga */}
...
### 個別設定 MCP Tool 的 FGA 範圍 {/* #scope-mcp-tool-fga-separately */}
...
### 在 Tools 中存取驗證資料 {/* #accessing-auth-data-in-tools */}
...
### 將 `RequestContext` 傳遞給 agent {/* #passing-requestcontext-through-to-agent */}
...
### `AppResources` 類型 {/* #appresources-type */}
