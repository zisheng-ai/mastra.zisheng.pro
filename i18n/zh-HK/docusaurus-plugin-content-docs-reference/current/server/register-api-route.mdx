...
<PropertiesTable
  ...
  },
  {
    name: 'handler',
    type: 'Handler',
    description: '接收 Hono Context 的 route handler function。只可使用 `handler` 或 `createHandler` 其中一項，不可同時使用。',
    isOptional: true,
  },
  {
    name: 'createHandler',
  ...
  },
  {
    ...
    type: 'CorsOptions',
    description:
      'Route 專用的 CORS 設定。當某個自訂 route 需要與 `server.cors` 不同的跨來源政策時使用。',
    isOptional: true,
  },
  {
    name: 'openapi',
