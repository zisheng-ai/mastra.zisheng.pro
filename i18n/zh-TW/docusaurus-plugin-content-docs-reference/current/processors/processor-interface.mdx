...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'ProcessorStreamWriter',
    description:
      '用於將自訂資料區塊發回用戶端的串流寫入器。呼叫 writer.custom() 即可發出 data-* 類型的區塊。可在串流期間使用。',
    isOptional: true,
  },
]}
/>
...
- 傳回 `null` 會捨棄區塊，不會傳送任何內容給下一個 processor 或用戶端。
...
若要替換 Tool 結果，請透過 `messageList.updateToolInvocation` 就地修改 `messageList`。執行階段會在 processor 執行後重新從訊息清單讀取結果，並在排入佇列前覆寫下游的 Tool 結果串流區塊，因此串流用戶端會看到處理後的值。
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'unknown',
    description:
      'Tool 傳回的值。若 Tool 由用戶端執行，此值是經過 `ensureSerializable` 處理後的 `tool.execute()` 輸出。若 Tool 由 Provider 執行（例如 Anthropic `web_search`），此值是 Provider 串流的原始結果，不會經過 `ensureSerializable` 處理。',
    isOptional: false,
  },
  {
    ...
    type: 'boolean',
    description:
      '此結果是否來自 Anthropic web_search 等由 Provider 執行的 Tool。對用戶端執行的 Tool 而言，預設為 undefined。',
    isOptional: true,
  },
  {
    name: 'systemMessages',
...
每個方法上的 `abort` 函式都會擲出 `TripWire` 錯誤，停止處理並在輸出串流中發出 `tripwire` 區塊。用戶端可偵測此區塊，以區分遭封鎖的回應與正常結束。
...
可存取 `writer` 的 processor 能呼叫 `writer.custom(chunk)`，將自訂 `data-*` 區塊串流傳送至用戶端。Tool 也能透過自己的 writer 執行相同作業。這是 processor 在一般文字及 Tool 區塊以外發出內容的唯一方式。
