...
## Processor 方法的執行時機 {/* #when-processor-methods-run */}
...
### `messages` 與 `messageList` 的比較 {/* #messages-vs-messagelist */}
...
<PropertiesTable
  ...
  },
  {
    name: 'systemMessages',
    type: 'CoreMessage[]',
    description: '所有系統訊息（Agent 指令、記憶體內容脈絡及使用者提供的內容）。可修改並傳回。',
    isOptional: false,
  },
  {
    name: 'messageList',
  ...
  },
  {
    name: 'retryCount',
    type: 'number',
    description: 'Processor 在此次產生內容時已觸發重試的次數。可用來限制重試次數。Mastra 一律會傳入此值，並從 0 開始。',
    isOptional: false,
  },
  {
    name: 'tracingContext',
...
#### Agentic loop 中的執行順序 {/* #execution-order-in-the-agentic-loop */}
...
<PropertiesTable
  ...
  },
  {
    name: 'retryCount',
    type: 'number',
    description: '來自 `ProcessorContext` 的目前重試次數。從 `0` 開始；可用來限制由 processor 觸發的重試。',
    isOptional: false,
  },
  {
    name: 'tracingContext',
...
<PropertiesTable
  ...
  },
  {
    name: 'abort',
    type: '(reason?: string, options?: { retry?: boolean; metadata?: unknown }) => never',
    description: '中止處理的函式。它會擲出 TripWire 錯誤、停止執行，並發出 `tripwire` 區塊。',
    isOptional: false,
  },
  {
    name: 'retryCount',
  ...
  },
  {
    ...
    description: '串流期間用於發出自訂資料區塊的串流寫入器。呼叫 `writer.custom()` 即可發出 `data-*` 區塊。',
    isOptional: true,
  },
  {
    name: 'abortSignal',
...
<PropertiesTable
  content={[
  {
    ...
    description: '此步驟中由 LLM 呼叫產生（或從快取重播）的區塊，採用精簡形式（`{ type, payload }`）。',
    isOptional: false,
  },
  {
    name: 'model',
...
<PropertiesTable
  ...
  },
  {
    name: 'retryCount',
    type: 'number',
    description: 'Processor 已觸發重試的次數。可用來限制重試次數。Mastra 一律會傳入此值，並從 0 開始。',
    isOptional: false,
  },
  {
    name: 'tracingContext',
...
#### 範例：可重試的品質防護機制 {/* #example-quality-guardrail-with-retry */}
...
<PropertiesTable
  ...
  },
  {
    name: 'abort',
    type: '(reason?: string, options?: { retry?: boolean; metadata?: unknown }) => never',
    description: '中止執行的函式。傳入 `retry: true`，可要求 LLM 以中止原因作為意見回饋來重試此步驟。',
    isOptional: false,
  },
  {
    name: 'retryCount',
...
#### 範例：遮蔽敏感欄位 {/* #example-redact-sensitive-fields */}
...
### 搭配 `processInputStep` 的逐步 processor {/* #per-step-processor-with-processinputstep */}
...
### 搭配 `processInputStep` 的訊息轉換 processor {/* #message-transformer-with-processinputstep */}
...
### 混合 processor（輸入與輸出） {/* #hybrid-processor-input-and-output */}
...
### 使用狀態的串流累加 processor {/* #stream-accumulator-with-state */}
...
## 中止與 tripwire 區塊 {/* #aborting-and-tripwire-chunks */}
...
## 發出自訂資料區塊 {/* #emitting-custom-data-chunks */}
...
## 在 Agent 上設定 processor {/* #configuring-processors-on-an-agent */}
