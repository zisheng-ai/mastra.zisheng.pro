    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            name: 'strategy',
            type: "'truncate' | 'abort'",
            description: "達到 token 限制時採用的策略：'truncate' 會停止發出區塊；'abort' 會呼叫 abort() 以停止串流",
            isOptional: true,
            default: "'truncate'",
          },
          {
...
<PropertiesTable
  ...
  },
  {
    ...
    description: '在 Agent 迴圈開始前篩選輸入訊息，使其符合 token 限制，優先保留近期訊息，同時保留系統訊息',
    isOptional: false,
  },
  {
...
### 作為輸入處理器（限制情境視窗） {/* #as-an-input-processor-limit-context-window */}
...
### 作為逐步輸入處理器（限制多步驟 token 增長） {/* #as-a-per-step-input-processor-limit-multi-step-token-growth */}
...
### 作為輸出處理器（限制回應長度） {/* #as-an-output-processor-limit-response-length */}
