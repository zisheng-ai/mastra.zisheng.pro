...
## 預設 OpenAI Responses 比對器

`isRetryableOpenAIResponsesStreamError` 會比對 `type: 'error'` 或 `type: 'response.failed'` 的 OpenAI Responses 串流錯誤區塊。它會重試已知的暫時性 OpenAI 錯誤代碼；若未比對到，則會重試具有明確重試指引的錯誤，例如 `You can retry your request`。
