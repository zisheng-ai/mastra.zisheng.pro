...
<PropertiesTable
  ...
  },
  {
    name: 'pollInterval',
    type: 'number',
    description: 'poll 間隔，以毫秒為單位。設定後，framework 會依此間隔呼叫 `poll()`。只使用 webhook 的 Provider 請保留為 `undefined` 或 `0`。',
    isOptional: true,
  },
  {
    name: 'isConnected',
    type: 'boolean',
    description: '此 Provider 是否已連線至 Agent。呼叫 `connect()` 後傳回 `true`。內部用於在 `Agent.__fork()` 期間略過重複接線。',
  },
]}
/>
