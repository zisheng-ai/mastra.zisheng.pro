    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            isOptional: true,
            description:
              '覆寫 Agent 上設定的輸出 processor。輸出 processor 可在訊息回傳給用戶前，修改或驗證 Agent 的訊息。必須實作 `processOutputResult` 或 `processOutputStream` 函式之一（也可兩者都實作）。',
          },
          {
          ...
          },
          {
            ...
            isOptional: true,
            description:
              "在請求的「用戶端」執行的 Tool。這些 Tool 的定義中沒有 execute 函式。",
          },
          {
            name: 'hooks',
            type: 'ToolHooks',
          ...
          },
          {
            ...
            isOptional: true,
            description:
              "設為 true 時，當用戶在同一個 thread 傳送新訊息，就會自動恢復已暫停的 Tool。Agent 會依據 Tool 的 `resumeSchema`，從用戶訊息擷取 `resumeData`。必須設定 memory。",
          },
          {
                ...
                parameters: [
                  {
                    ...
                    isOptional: true,
                    description:
                      '要加入根 trace span 的中繼資料。適合用來加入用戶 ID、session ID 或功能旗標等自訂屬性。',
                  },
                ],
              },
              {
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      '啟用 Tracing 時，與這次執行關聯的 trace ID。可用來關聯日誌及偵錯執行流程。',
  },
  {
    name: 'spanId',
    type: 'string',
