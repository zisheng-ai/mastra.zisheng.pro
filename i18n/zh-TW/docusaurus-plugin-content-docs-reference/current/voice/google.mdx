    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'string',
            description:
              "語音的語言代碼（例如 'en-US'）。預設使用從語音 ID 衍生的語言代碼，若無則使用 'en-US'。",
            isOptional: true,
          },
          {
            name: 'input',
    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            type: 'string',
            description:
              "v2 辨識器資源路徑。預設為 `projects/{project}/locations/global/recognizers/_`，其中 `{project}` 會從建構函式的 `project` 選項、`GOOGLE_CLOUD_PROJECT` 或用戶端的預設專案解析。",
            isOptional: true,
          },
        ],
      },
...
1. 可使用 `getSpeakers()` 方法依語言代碼篩選可用語音。
