...
<PropertiesTable
  content={[
  {
    name: 'apiKey',
    type: 'string',
    description: '你的 WorkOS API key。此 key 用於向 WorkOS API 進行身分驗證，以驗證使用者及管理組織。',
    isOptional: true,
  ...
  },
  {
    name: 'clientId',
    type: 'string',
    description: '你的 WorkOS Client ID。在用 authorization code 交換 access token 時，此值用於識別你的應用程式。',
    isOptional: true,
