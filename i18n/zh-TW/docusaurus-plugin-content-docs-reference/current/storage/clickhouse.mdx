...
預設值符合最常見的自主管理慣例。若 cluster 的現有資料表使用不同版面配置（例如不含 `{database}` 區段的 `/clickhouse/tables/{shard}/{table}`），請明確設定相符的 `zookeeperPath`。Mastra 不會從 Keeper 讀取 cluster 慣例，因此不相符的預設值會將 Mastra metadata 寫入與 cluster 其餘部分不同的 branch。
