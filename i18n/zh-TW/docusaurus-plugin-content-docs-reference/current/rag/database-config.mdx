...
<PropertiesTable
  ...
  },
  {
    ...
    description: '用於混合搜尋的稀疏向量，可結合密集與稀疏嵌入，提升關鍵字查詢的搜尋品質。indices 與 values 陣列的長度必須相同。',
    isOptional: true,
    properties: [
      {
        type: 'object',
...
<PropertiesTable
  ...
  },
  {
    name: 'ef',
    type: 'number',
    description: 'HNSW 搜尋參數，用來控制搜尋期間動態候選清單的大小。較高的值會以速度為代價提升準確度。通常設在 topK 到 200 之間。',
    isOptional: true,
  },
  {
    name: 'probes',
...
<PropertiesTable
  content={[
  {
    ...
    description: '查詢的一致性層級。"strong"（預設）可保證查詢看得到查詢開始前寫入的所有資料，但延遲較高。"eventual" 的延遲較低，但近期寫入的資料可能尚不可見。',
    isOptional: true,
  },
]}
/>
