...
description: "`DatasetsManager.compareExperiments()` 方法的說明文件，該方法比較兩個或更多實驗。"
packages:
  - "@mastra/core"
---
...
比較兩個或更多實驗，產生依項目和依 Scorer 區分的比較結果。必須提供至少兩個實驗 ID。
...
<PropertiesTable
  content={[
  {
    name: 'experimentIds',
    type: 'string[]',
    description: '要比較的實驗 ID 陣列。必須至少包含 2 個。',
  },
  {
    name: 'baselineId',
    type: 'string',
    isOptional: true,
    description: '基線實驗的 ID。預設為 `experimentIds` 中的第一個 ID。',
  },
]}
/>
...
如果提供的實驗 ID 少於 2 個，則擲回 `MastraError`。
            ...
            properties: [
              {
                ...
                parameters: [
                  ...
                  },
                  {
                    name: 'groundTruth',
                    type: 'unknown | null',
                    description: '項目的 ground truth。',
                  },
                  {
                    name: 'results',
                    type: 'Record<string, { output: unknown; scores: Record<string, number | null> } | null>',
                    description: '以實驗 ID 為鍵的結果。每個項目包含該實驗的輸出和 Scorer 結果。',
                  },
                ],
              },
            ],
