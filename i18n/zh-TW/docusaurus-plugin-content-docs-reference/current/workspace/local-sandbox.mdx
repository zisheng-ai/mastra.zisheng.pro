...
`LocalSandbox` 內建程序管理器，可產生及管理背景程序。這些程序會透過 `child_process.spawn`，以子程序形式在本機執行。
...
啟用原生隔離（`seatbelt` 或 `bwrap`）時，產生的程序也會以相同的隔離後端包裝。
...
此分隔方式可防止 Sandbox 中的程序讀取或修改自己的安全性 profile。Profile 會在 Sandbox 啟動時建立，並在銷毀時清除。
