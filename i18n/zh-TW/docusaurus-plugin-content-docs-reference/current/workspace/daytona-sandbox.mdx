...
<PropertiesTable
  ...
  },
  {
    ...
    description: 'Sandbox 的資源配置（CPU 核心數、以 GiB 為單位的記憶體與磁碟）。只有在設定 image 時才會使用。',
    isOptional: true,
  },
  {
    name: 'env',
...
`DaytonaSandbox` 內建程序管理器，可產生及管理背景程序。程序使用以工作階段為基礎的指令執行方式，在 Daytona 雲端 Sandbox 中執行。
