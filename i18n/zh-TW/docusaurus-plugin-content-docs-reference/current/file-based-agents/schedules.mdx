...
<PropertiesTable
  content={[
  { name: 'cron', type: 'string', description: '標準五欄 cron expression。必填。scheduler 會在 tick loop 上評估 Schedule，因此有效精細度為一分鐘。不支援小於一分鐘的欄位。' },
  { name: 'prompt', type: 'string', description: '每次觸發時 Agent 執行的訊息。設定此欄位或 `handler`，不可同時設定。', isOptional: true },
  { name: 'handler', type: '(ctx) => ScheduleOverrides | null | undefined', description: '在觸發時計算該次執行。傳回要套用的 override，或傳回 `null` 略過該次觸發。未傳回內容不會套用 override，而 handler 模式沒有已儲存的 prompt，因此會造成觸發失敗。設定此欄位或 `prompt`，不可同時設定。', isOptional: true },
  { name: 'timezone', type: 'string', description: '評估 cron 時使用的 IANA timezone，例如 `America/New_York`。預設為 host process timezone，可能因部署而異，因此任何對時刻敏感的工作都應明確設定。timezone 規則會處理 DST 轉換，所以 `0 9 * * *` 在轉換前後都維持當地時間上午 9 點。', isOptional: true },
  { name: 'name', type: 'string', description: '顯示於 Studio，且可透過 `mastra.schedules.list({ name })` 篩選的自由格式 label。', isOptional: true },
  { name: 'threadId', type: 'string', description: '將觸發以 signal 形式傳入現有 thread，而不啟動新的 run。需要 `resourceId`。', isOptional: true },
  { name: 'resourceId', type: 'string', description: '目標 thread 的擁有者。設定 `threadId` 時必填。', isOptional: true },
  { name: 'signalType', type: "'user' | 'state' | 'reactive' | 'notification' | 'user-message' | 'system-reminder'", description: '該次觸發的 signal category。只適用於 thread 型 Schedule。', isOptional: true, defaultValue: "'notification'" },
  { name: 'tagName', type: 'string', description: 'signal render 使用的 XML tag，因此觸發會以 `<schedule>…</schedule>` 形式送達 Agent。', isOptional: true, defaultValue: "'schedule'" },
  { name: 'attributes', type: 'Record<string, string | number | boolean | null>', description: 'render 至 signal XML tag 的 attribute。', isOptional: true },
  { name: 'providerOptions', type: 'Record<string, unknown>', description: '每次觸發時合併至 Schedule signal payload 的 Provider 選項。必須可安全轉換為 JSON。', isOptional: true },
  { name: 'ifActive', type: 'ScheduleIfActive', description: '目標 thread 已在串流時的處理方式：`deliver`、`persist` 或 `discard`。只適用於 thread 型 Schedule。', isOptional: true },
  { name: 'ifIdle', type: 'ScheduleIfIdle', description: '目標 thread 閒置時的處理方式：`wake`、`persist` 或 `discard`。只適用於 thread 型 Schedule。', isOptional: true },
  { name: 'status', type: "'active' | 'paused'", description: '建立 row 時使用的狀態。只在首次建立時套用，因為同步永遠不會 patch `status`，讓透過 API 暫停的狀態可在重新部署後保留。日後在程式碼中變更此值，不會影響現有 Schedule。', isOptional: true, defaultValue: "'active'" },
  { name: 'metadata', type: 'Record<string, unknown>', description: '與 Schedule row 一起儲存、可安全轉換為 JSON 的任意資料。', isOptional: true },
]}
/>
