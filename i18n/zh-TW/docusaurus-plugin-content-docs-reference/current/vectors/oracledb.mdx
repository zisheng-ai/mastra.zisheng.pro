...
預設為 `indexConfig: { type: 'none' }`，使用精確搜尋且不需調校近似索引。只有當資料量與延遲需求足以支援使用近似搜尋時，才使用 IVF 或 HNSW。HNSW 透過 `indexConfig: { type: 'hnsw', hnsw: { neighbors, efConstruction } }` 設定，且需要 Oracle Vector Pool 記憶體；本機或自行管理的資料庫可使用 `configureVectorMemory()` 配置該記憶體。
...
配置 HNSW 索引所需的 Oracle Vector Pool 記憶體。此方法會呼叫 `ALTER SYSTEM SET VECTOR_MEMORY_SIZE`，因此需要 `SYSDBA` 或 `SYSTEM` 等具權限的連線。
