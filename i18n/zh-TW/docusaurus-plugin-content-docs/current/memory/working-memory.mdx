...
## Memory 保存作用域 {/* #memory-persistence-scopes */}

Working memory 可在兩種不同的作用域中運作，讓你選擇跨對話保存 Memory 的方式：

### Resource 作用域 Memory（預設） {/* #resource-scoped-memory-default */}
...
## 儲存 adapter 支援 {/* #storage-adapter-support */}

Resource 作用域 working memory 需要支援 `mastra_resources` 資料表的特定儲存 adapter：

### 支援的儲存 adapter {/* #supported-storage-adapters */}
...
## 結構化 working memory {/* #structured-working-memory */}
...
### 範例：以 schema 為基礎的 working memory {/* #example-schema-based-working-memory */}
...
### 以 schema 為基礎之 Memory 的合併語意 {/* #merge-semantics-for-schema-based-memory */}
...
## 範例：跨多步驟保留 {/* #example-multi-step-retention */}
...
## 設定初始 working memory {/* #setting-initial-working-memory */}
...
### 透過 thread metadata 設定 working memory {/* #setting-working-memory-via-thread-metadata */}
...
### 以程式更新 working memory {/* #updating-working-memory-programmatically */}
...
## 唯讀 working memory {/* #read-only-working-memory */}
...
## 選擇使用狀態訊號（實驗性） {/* #opt-in-to-state-signals-experimental */}
