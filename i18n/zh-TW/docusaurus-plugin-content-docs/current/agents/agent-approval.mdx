...
Agent 呼叫處理敏感操作（例如刪除資源或執行耗時流程）的 Tool 時，有時也需要 Workflow 所使用的[人機協作](/docs/workflows/human-in-the-loop)監督機制。透過 Agent 核准，你可以在 Tool 呼叫執行前將其暫停，讓人員核准或拒絕；也可以讓 Tool 自行暫停，以向使用者要求額外資訊。

## 何時使用 Agent 核准 {/* #when-to-use-agent-approval */}
...
## 核准的運作方式 {/* #how-approval-works */}

Mastra 提供兩種不同的 Tool 呼叫暫停機制：**執行前核准**與**執行階段暫停**。

### 執行前核准 {/* #pre-execution-approval */}

執行前核准會在 Tool 呼叫的 `execute` 函式執行\_之前\_將其暫停。LLM 仍會決定要呼叫哪個 Tool 並提供引數，但在你明確核准之前，`execute` 不會執行。

下列旗標以 OR 邏輯共同控制此行為。只要\_任一\_旗標為 `true`，呼叫就會暫停：
...
#### 使用函式進行條件式核准 {/* #conditional-approval-with-a-function */}
...
#### 將核准綁定至確切的 Tool 引數 {/* #bind-approval-to-the-exact-tool-arguments */}
...
Tool 也可以在其 `execute` 函式執行\_期間\_呼叫 `suspend()` 來暫停。當 Tool 開始執行後才發現需要更多使用者輸入或確認才能完成時，此功能相當實用。
...
## 搭配 `generate()` 使用 Tool 核准 {/* #tool-approval-with-generate */}
...
### 串流與產生結果的比較 {/* #stream-vs-generate-comparison */}
...
### 使用 `suspend()` 核准 {/* #approval-using-suspend */}
...
### 手動與自動恢復的比較 {/* #manual-vs-automatic-resumption */}
...
## 重新啟動後恢復執行 {/* #resuming-after-a-restart */}
...
只有在 Mastra 執行個體設定了永久性[儲存 Provider](/docs/storage/overview)時，已暫停的執行才能在重新啟動後繼續存在。預設的記憶體內儲存區會在處理程序結束時遺失快照。

:::

## Tool 核准：Supervisor Agent {/* #tool-approval-supervisor-agents */}
...
### 在 Supervisor Agent 中核准與拒絕 {/* #approve-and-decline-in-supervisor-agents */}
...
### 搭配 `generate()` 使用 Supervisor 核准 {/* #supervisor-approval-with-generate */}
