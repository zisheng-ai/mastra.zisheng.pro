---
title: "Langfuse Exporter | 整合 | Observability"
description: "將 Trace 傳送至 Langfuse，以進行 LLM Observability 分析及資料分析"
packages:
  - "@mastra/langfuse"
...
[Langfuse](https://langfuse.com/) 是專為 LLM 應用程式而設的開源 Observability 平台。Langfuse Exporter 會將你的 Trace 傳送至 Langfuse，讓你深入了解模型效能、token 用量及對話流程。
...
1. **Langfuse 帳戶**：在 [cloud.langfuse.com](https://cloud.langfuse.com) 註冊，或自行代管部署
...
設定環境變數後，便可使用 Exporter，無須提供任何設定：
...
### 即時模式與批次模式 {/* #realtime-vs-batch-mode */}

Langfuse Exporter 支援以兩種模式傳送 Trace：

#### 即時模式（開發環境） {/* #realtime-mode-development */}
...
#### 批次模式（正式環境） {/* #batch-mode-production */}
...
#### 為大量 Trace 調整批次設定 {/* #batch-tuning-for-high-volume-traces */}

如自行代管 Langfuse，或串流執行每秒產生大量 span，你可以調整 OTEL 批次大小及 flush 間距，以減輕 Langfuse 資料擷取端點的請求壓力：
...
如要完全排除大量出現的 span 類型（例如串流回應中的 `MODEL_CHUNK` span），請使用 Observability 層級的 [`excludeSpanTypes` 選項](/reference/observability/tracing/span-filtering)，而非設定 Exporter：
...
## 按 Agent 限定 evaluator 的範圍 {/* #scoping-evaluators-per-agent */}
...
Root span 為 `WORKFLOW_RUN` 時也同樣適用，並會設定 `langfuse.trace.metadata.workflowId` 及 `langfuse.trace.metadata.workflowName`。

如要將 evaluator 的範圍限定於特定 Agent，請在 Langfuse 設定以下其中一項篩選條件：
...
## 自訂 Trace metadata {/* #custom-trace-metadata */}
...
### 使用 helper（建議） {/* #using-the-helper-recommended */}
