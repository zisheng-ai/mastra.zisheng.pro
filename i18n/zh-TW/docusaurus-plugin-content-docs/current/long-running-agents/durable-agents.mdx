...
description: "瞭解如何用持久型執行包裝 Agent，讓串流能承受連線中斷、程序重新啟動及長時間執行的輪次。"
packages:
  - "@mastra/core"
  - "@mastra/inngest"
---
...
持久型 Agent 會包裝一般的 [`Agent`](/docs/agents/overview)，讓 Agent 迴圈在 Workflow 內執行。事件透過 [PubSub](/docs/server/pubsub) 傳遞，因此用戶端可以中斷連線後重新連線，而不會遺漏區塊。執行狀態會持久化，因此能承受程序重新啟動。
...
| 工廠 | 套件 | 最適合的用途 |
| - | - | - |
| `createDurableAgent()` | `@mastra/core` | 本機開發與單一程序伺服器。你會取得可直接 await 的串流。 |
...
包裝 Agent 並呼叫 `stream()`，即可在同一程序中取得 `DurableAgentStreamResult`。此方式不需要外部基礎架構，因此是最快的入門方式：
...
`createDurableAgent()` 和 `createEventedAgent()` 預設使用記憶體內快取，表示可恢復串流僅在單一程序內運作。正式環境請提供持久型快取後端（例如 Redis），使快取事件能承受程序重新啟動：
...
若持久型 Agent 執行期間伺服器程序當機，該次執行會在儲存空間中維持 `running` 狀態，且不會自動重試。下次伺服器啟動時，你可以重新驅動這些孤立的執行，讓它們從中斷處繼續。
