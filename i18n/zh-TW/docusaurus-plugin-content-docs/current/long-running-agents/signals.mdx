...
訊號會透過 pub/sub 後端協調執行。訊號抵達實作 `LeaseProvider` 的後端時，Mastra 會取得目標對話串的 Lease，使同一時間只有一個程序擁有該對話，再喚醒 Agent 或將輸入導入執行中迴圈。不具 Lease 功能的後端會退回至一律授予擁有權的空操作；這適合單一程序，但不適合跨執行個體使用。
