...
### `createTool` execute 簽章改為 `(inputData, context)` 格式 {/* #createtool-execute-signature-to-inputdata-context-format */}
...
### `createTool` 內容屬性的組織方式 {/* #createtool-context-properties-organization */}
...
### `tool.execute` 傳回型別包含 `ValidationError` {/* #toolexecute-return-type-includes-validationerror */}
