...
Worker 是長時間執行的 Node.js 程序，會輪詢 Temporal 工作佇列。安裝 `MastraPlugin`，並將其 `src` 選項指向註冊 Workflow 的 Mastra 進入點檔案。
...
1. 從指令碼或任何匯入 Mastra 執行個體的程序觸發 Workflow 執行：
...
:::warning
Temporal Worker 必須以長時間執行的程序運作。請勿將其部署至 AWS Lambda 或 Vercel 函式等執行時間受限的無伺服器平台。請使用容器、虛擬機器，或 Fly.io、Railway、Kubernetes 等適合 Worker 的平台。
:::
