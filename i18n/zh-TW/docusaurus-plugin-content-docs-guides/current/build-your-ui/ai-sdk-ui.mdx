...
## 使用 Mastra Memory {/* #using-mastra-memory */}

當 Agent 已設定 [memory](/docs/memory/overview) 時，Mastra 會從伺服器上的儲存空間載入對話歷程。使用者端只需傳送新訊息，不要傳送完整對話歷程。
...
[`chatRoute()`](/reference/ai-sdk/chat-route) 與 [`handleChatStream()`](/reference/ai-sdk/handle-chat-stream) 已支援 memory。請設定使用者端只傳送新訊息，並包含 thread 與 resource 識別碼。
...
### 轉譯 Tool 輸出 {/* #rendering-tool-outputs */}
...
### 轉譯 Workflow 資料 {/* #rendering-workflow-data */}
...
### 轉譯 network 資料 {/* #rendering-network-data */}
...
### 由使用者核准 Workflow 暫停／繼續 {/* #workflow-suspendresume-with-user-approval */}
...
### Tool 中的巢狀 Agent 串流 {/* #nested-agent-streams-in-tools */}
...
### 從 Workflow step 串流 Agent 文字 {/* #streaming-agent-text-from-workflow-steps */}
...
### Agent network 中的進度指示器 {/* #progress-indicators-in-agent-networks */}
