...
description: 使用 GitHub Actions Workflow 自動產生多語言 Pull Request 描述；此 Workflow 使用 Mastra 讀取 diff，並產生精簡的 PR 描述。
---

# 為 GitHub Actions 建立 PR 描述 Agent

本指南將帶你建立一個 [GitHub Action](https://docs.github.com/en/actions)，使用 Mastra Agent 讀取 Pull Request 的 diff、產生多語言摘要，並直接寫入 GitHub UI 中的 PR 描述。每當 Pull Request 建立或更新時，此 Action 都會執行，並隨著 diff 的變更重新產生描述。

這種做法類似 [CodeRabbit](https://coderabbit.ai/) 和 [Greptile](https://www.greptile.com/) 等 AI 驅動的 PR Tool，但使用原生 GitHub Actions Workflow 與 Mastra Agent 建立。
...
- Mastra 專案（請參閱[快速入門](/guides/getting-started/quickstart)）
- 支援的 [Model Provider](/models) 所提供的 API 金鑰。如果沒有偏好，請使用 [OpenAI](/models/providers/openai)
- Node.js `v22.13.0` 或更新版本

### 設定儲存庫 Secret {/* #set-up-repository-secrets */}

Workflow 需要存取 Model Provider 的 API 金鑰。請在 GitHub 中將其新增為儲存庫 Secret：

1. 前往 GitHub 上的儲存庫
1. 依序前往 **Settings** > **Secrets and variables** > **Actions**
1. 按一下 **New repository secret**
1. 新增 API 金鑰，名稱設為 `OPENAI_API_KEY`（若使用其他 Provider，則設為對應的金鑰名稱）

   ![GitHub UI 中儲存庫 Secret 的螢幕截圖](/img/guides/github-action-repository-secrets.jpg)
...
部署 Workflow 之前，你可以在 [Studio](/docs/studio/overview) 中測試 Agent，確認它能正確產生描述。
...
1. 在 Studio 中開啟 PR Description Agent，貼上 diff 內容並使用以下提示：
...
### 建立 Workflow 指令碼 {/* #create-the-workflow-script */}

在 `.github` 內建立 `scripts` 目錄，並新增 `generate-description.ts`。
...
#### Workflow 指令碼說明 {/* #the-workflow-script-explained */}
...
- 為 Agent 提供 [Tool](/docs/agents/using-tools)，以擷取更多背景資訊
- 探索其他 [GitHub Actions 觸發條件](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows)，例如 issue 留言或發布版本
