---
title: "指南：WhatsApp 聊天機器人"
description: 使用 Mastra Agent 與 Workflow 建立 WhatsApp 聊天機器人，以處理傳入訊息並透過文字訊息自然回覆。
---

# WhatsApp 聊天機器人

本指南示範如何使用 Mastra Agent 與 Workflow 建立 WhatsApp 聊天機器人。機器人會透過 webhook 接收傳入的 WhatsApp 訊息、交由 AI Agent 處理、將回覆拆分成自然的文字訊息，再透過 WhatsApp Business API 傳送回去。
...
## 建立 WhatsApp client {/* #creating-the-whatsapp-client */}

此 client 負責透過 WhatsApp Business API 將訊息傳送給使用者。
...
## 建立聊天 Agent {/* #creating-the-chat-agent */}

此 Agent 以友善、自然對話的個性處理主要對話邏輯。
...
## 建立文字訊息 Agent {/* #creating-the-text-message-agent */}

此 Agent 會將較長的回覆轉換成適合 WhatsApp、自然且簡短易讀的文字訊息。
...
## 建立聊天 Workflow {/* #creating-the-chat-workflow */}

此 Workflow 會協調整個聊天流程：生成回覆、將回覆拆分成多則訊息，再透過 WhatsApp 傳送。
...
## 設定 Mastra {/* #setting-up-mastra-configuration */}

使用 Agent、Workflow 與 WhatsApp webhook endpoint 設定 Mastra instance。
...
## 測試聊天機器人 {/* #testing-the-chat-bot */}

你可以模擬 WhatsApp webhook payload，在本機測試聊天機器人。
...
當使用者將「Hello! How are you today?」傳送給 WhatsApp 機器人時，機器人可能會以多則訊息回覆，例如：
...
機器人會透過 Memory 維持對話情境，並提供符合 WhatsApp 訊息風格的自然回覆。
