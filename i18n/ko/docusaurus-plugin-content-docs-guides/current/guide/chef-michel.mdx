...
<Steps>
  <StepItem>
    새 파일 만들기`src/index.ts` and add a `main()` 함수를 추가합니다. 함수 내부에서 Agent에 전달할 쿼리를 작성하고 응답을 로그에 기록합니다.
...
<Steps>
  ...
  <StepItem>
    기본적으로`mastra dev` runs on `http://localhost:4111`. Chef Assistant Agent는 다음 주소에서 사용할 수 있습니다:
