...
원래의 Transformer Paper를 사용하게 됩니다.["Attention Is All You Need"](https://arxiv.org/html/1706.03762) as your example. As a database you'll use a local libSQL database.
...
* 텍스트를 벡터 표현으로 변환
* 콘텐츠의 숫자 표현 만들기
* **구현**: OpenAI를 사용하게 됩니다.`text-embedding-3-small` to create embeddings and store them in LibSQLVector
...
<Steps>
  <StepItem title="Install additional dependencies">
    ...
    실행 후[installation guide](/guides/getting-started/quickstart) you'll need to install additional dependencies:
  ...
  <StepItem title="Define the Agent">
    ...
    * 에이[Vector Query Tool](/reference/tools/vector-query-tool) for performing semantic search over the vector store to find relevant content in papers
    * 쿼리를 이해하고 응답을 생성하기 위한 OpenAI Model
    * 서류를 분석하고, 검색된 콘텐츠를 효과적으로 사용하고, 한계를 인정하는 방법을 Agent에게 안내하는 맞춤형 지침

    새 파일 만들기`src/mastra/agents/researchAgent.ts` and define your agent:
  ...
  <StepItem title="Create the Vector Store">
    프로젝트 루트에서 다음을 사용하여 절대 경로를 확보하십시오.`pwd` command. The path might be similar to this:
    ...
    당신의`src/mastra/index.ts` file, add the following to your existing file and configuration:
    ...
    에 대한`url` use the absolute path you got from the `pwd` command. This way the `vector.db` file is created at the root of your project.
  ...
  <StepItem title="Register the Agent with Mastra">
    에서`src/mastra/index.ts` file, add the agent to Mastra:
...
<Steps>
  <StepItem title="Load and Process the Paper">
    ...
    새 파일 만들기`src/store.ts` and add the following:
  ...
  <StepItem title="Create and Store Embeddings">
    ...
    열기`src/store.ts` file and add the following:
...
새 파일 만들기`src/ask-agent.ts` and add different types of queries:
