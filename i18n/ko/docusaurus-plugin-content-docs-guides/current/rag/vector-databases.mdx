...
<Tabs>
  ...
  <TabItem value="oracledb" label="OracleDB">
    ...
    * 예:`customer-support/docs:v1` 가 유효하며 안전한 Oracle 테이블 이름으로 매핑됩니다
  </TabItem>
...
RAG 애플리케이션을 구축할 때 문서가 삭제되거나 업데이트될 때 오래된 벡터를 정리해야 하는 경우가 많습니다. 마스트라가 제공하는`deleteVectors` 메서드는 메타데이터 필터를 기준으로 벡터 삭제를 지원하므로 특정 문서와 연결된 모든 임베딩을 간편하게 제거할 수 있습니다.
