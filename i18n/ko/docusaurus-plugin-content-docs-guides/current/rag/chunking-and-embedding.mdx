...
사용`chunk` to split documents into manageable pieces. Mastra supports multiple chunking strategies optimized for different document types:
...
임베딩 Model은 일반적으로 고정된 수의 차원(예: OpenAI의 경우 1536)으로 벡터를 출력합니다.`text-embedding-3-small`).
