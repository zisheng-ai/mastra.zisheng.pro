...
그만큼`topK` parameter specifies the maximum number of most similar results to return from the vector search.
...
때로는 Agent에 벡터 데이터베이스를 직접 쿼리할 수 있는 기능을 부여하고 싶을 때가 있습니다. 벡터 쿼리 Tool를 사용하면 Agent가 사용자 요구 사항에 대한 Agent의 이해를 바탕으로 의미론적 검색과 선택적 필터링 및 순위 재지정을 결합하여 검색 결정을 담당할 수 있습니다.
...
Tool를 생성할 때 Tool 이름과 설명에 특별한 주의를 기울이십시오. 이는 Agent가 검색 기능을 사용하는 시기와 방법을 이해하는 데 도움이 됩니다. 예를 들어, 이름을 "SearchKnowledgeBase"로 지정하고 "X 주제에 대한 관련 정보를 찾으려면 문서를 검색하세요."라고 설명할 수 있습니다.
...
벡터 쿼리 Tool는 다양한 벡터 저장소의 고유한 기능과 최적화를 사용할 수 있도록 하는 데이터베이스별 구성을 지원합니다.

:::참고
이러한 구성은**query-time options** like namespaces, performance tuning, and filtering, not for database connection setup.
