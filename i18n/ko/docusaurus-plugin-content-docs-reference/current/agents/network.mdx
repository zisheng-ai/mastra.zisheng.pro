...
그만큼`.network()` primitive has been deprecated and will be removed in a future major release. Use [supervisor agents](/docs/capabilities/subagents) with `agent.stream()`또는`agent.generate()`대신에. 참조[마이그레이션 가이드](/guides/migrations/network-to-supervisor)업그레이드합니다.
...
네트워크에서 입력되고 검증된 결과가 필요한 경우`structuredOutput` option. The network will generate a response matching your schema after task completion.
...
사용`abortSignal` to cancel a running network. When aborted, the network stops routing, cancels any in-progress sub-agent, tool, or workflow execution, and doesn't save partial results to memory.
