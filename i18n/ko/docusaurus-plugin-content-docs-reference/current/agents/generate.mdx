...
`Agent.generate()`실행 중에 수집된 최종 데이터를 반환합니다.`steps` is an array of step objects. The tool arrays in the result, including top-level `toolCalls` and `toolResults` and the nested `step.toolCalls`그리고`step.toolResults`배열의 경우 Mastra의 청크 형식을 사용하세요.
...
Agent는 시각적 콘텐츠와 그 안의 텍스트를 모두 처리하여 이미지를 분석하고 설명할 수 있습니다. 이미지 분석을 활성화하려면 다음을 사용하여 객체를 전달하세요.`type: 'image'` and the image URL in the `content` array. You can combine image content with text prompts to guide the agent's analysis.
...
다음을 사용하여 다단계 작업의 진행 상황을 모니터링할 수 있습니다.`onStepFinish` callback. This is useful for debugging or providing progress updates to users.
...
언제`generateTitle` is enabled in memory options, title generation runs asynchronously after the response completes. Use `onTitleGenerated` to react when the title is ready, for example by pushing it to the client via SSE.
