...
The `MastraModelOutput` class is returned by [.stream()](./stream) 이며 Model 출력에 대한 스트리밍 방식과 Promise 기반 접근을 모두 제공합니다. 구조화된 출력 생성, Tool 호출, 추론 및 상세한 사용량 추적을 지원합니다.
...
다음 속성은 Model 출력이 생성되는 동안 실시간으로 접근할 수 있게 해 줍니다:
...
## Promise-based properties {/* #promise-based-properties */}

다음 속성은 스트림이 완료된 후 최종 값으로 결정됩니다:
...
### Basic Text Streaming {/* #basic-text-streaming */}
...
- [ChunkType](../ChunkType): 전체 스트림에서 가능한 모든 청크 유형
