...
Workflow를 정의할 수 있습니다.`inputSchema` and `outputSchema` with any library that supports [Standard JSON Schema](https://standardschema.dev/json-schema). 여기에는 다음과 같은 라이브러리가 포함됩니다.[조드](https://zod.dev/), [발리봇](https://valibot.dev/), 그리고[방주 유형](https://arktype.io/).
...
Workflow 실행을 시작할 때 다음을 전달할 수 있습니다.`initialState` to set the starting values for the workflow's state:
...
Workflow의`status` indicates its current execution state. The possible values are:
...
Workflow에 트립와이어를 트리거하는 Agent 단계가 포함되어 있으면 Workflow는 다음과 같이 반환됩니다.`status: 'tripwire'` and includes tripwire details:
...
이는 다음과 구별됩니다.`status: 'failed'` which indicates an unexpected error. A tripwire status means a processor intentionally stopped execution (e.g., for content moderation).
