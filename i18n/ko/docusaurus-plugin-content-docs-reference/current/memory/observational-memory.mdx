...
관찰 기억(OM)은 긴 맥락의 Agent 기억을 위한 Mastra의 기억 시스템입니다. 안**Observer** watches conversations and creates observations. A **Reflector**관련 항목을 결합하고 중요한 패턴을 압축하여 관찰 내용을 재구성합니다. 이들은 함께 증가함에 따라 원시 메시지 기록을 대체하는 관찰 로그를 유지 관리합니다.
...
그만큼`observationalMemory` option accepts `true`, a configuration object, or `false`. Setting `true`OM을 활성화합니다.`__GATEWAY_GOOGLE_MODEL_FLASH__`기본 Model로. 구성 객체를 전달할 때 다음을 설정합니다.`model`최상위 레벨이나`observation.model`및/또는`reflection.model`; 모든 Model 필드가 생략되면 OM은 다음으로 대체됩니다.`__GATEWAY_GOOGLE_MODEL_FLASH__`.

관찰자 입력은 다중 모드를 인식합니다. OM은 다음과 같은 텍스트 자리 표시자를 유지합니다.`[Image #1: screenshot.png]` in the transcript it builds for the Observer, and also sends the underlying image parts when possible. This applies to both single-thread observation and batched multi-thread observation. Non-image files appear as placeholders only.

OM은 빠른 로컬 토큰 추정으로 임계값을 수행합니다. 텍스트 사용`tokenx`, and image-like inputs use provider-aware heuristics plus deterministic fallbacks when metadata is incomplete.
...
* 부품 수준 캐시:`part.providerMetadata.mastra`.
* 문자열 콘텐츠 대체 캐시: 부분이 없는 경우 메시지 수준 메타데이터입니다.
* 캐시 버전/토크나이저 소스가 일치하지 않으면 캐시 항목이 무시되고 다시 계산됩니다.
* 메시지별 및 대화별 오버헤드는 항상 런타임에 다시 계산되며 캐시되지 않습니다.
* `data-*`그리고`reasoning` parts are skipped and don't receive cache entries.

## 추출기 API {/* #extractor-api */}

`Extractor`관찰 또는 반영 중에 OM이 추출해야 하는 값을 정의합니다. 다음과 같은 내장 OM 값`current-task`, `suggested-response`, and `thread-title` use the same extractor pipeline as custom values.
...
* 추출된 값은 다음 스레드 OM 메타데이터에 저장됩니다.`om.extracted`.
* 내장 추출기 값도 호환성 메타데이터 필드에 미러링됩니다.`currentTask`, `suggestedResponse`, and `threadTitle`.
* `thread-title`다음 경우에만 스레드 제목을 업데이트합니다.`observation.threadTitle` is enabled.
* `observation.extract`관찰 중에 실행됩니다.`reflection.extract` runs during reflection.
* 스키마 지원 추출기는 구조화된 후속 출력 요청을 추가합니다.
* 스키마 없는 추출기는 Observer 또는 Reflector 출력에서 ​​직접 방출되는 인라인 문자열 추출기입니다.
* 동적 추출기 함수는 다음을 포함하여 런타임 컨텍스트를 수신합니다.`source`, `threadId`, `resourceId`, `mainAgent`, `memory`, and `requestContext` when available.
* `WorkingMemoryExtractor`활성 추출기를 통해 작업 Memory를 업데이트하기 위해 일반 추출기 파이프라인을 사용합니다.`Memory` instance. It uses structured extraction when working memory has a JSON schema and skips OM metadata persistence, so the working memory payload isn't duplicated under OM extracted metadata.
* `observationalMemory.observation.manageWorkingMemory`추가하다`WorkingMemoryExtractor` and defaults `workingMemory.agentManaged` to `false`. It defaults `workingMemory.useStateSignals` to `true` when working memory is enabled.
* 추출 실패는 OM 마커 데이터에 보고되며 다른 성공적인 추출 값은 삭제되지 않습니다.

## 예 {/* #examples */}

### 작업 기억 업데이트 {/* #working-memory-updates */}

사용`observationalMemory.observation.manageWorkingMemory` when OM should update working memory.
...
세트`workingMemory.agentManaged: true` if the main agent should still receive working memory tool and instruction injection.
...
언제`shareTokenBudget` is enabled, the total budget is `observation.messageTokens + reflection.observationTokens` (100k in this example). If observations only use 30k tokens, messages can expand to use up to 70k. If messages are short, observations have more room before triggering reflection.
...
전달함으로써`model` in the config, you can use any model from Mastra's model router.
...
비동기 버퍼링은**enabled by default**. It pre-computes observations in the background as the conversation grows: when the `messageTokens` threshold is reached, buffered observations activate instantly with no blocking LLM call.

수명주기는 다음과 같습니다**buffer → activate → remove messages → repeat**. Background Observer calls run at `bufferTokens` intervals, each producing a chunk of observations. At threshold, chunks activate: observations move into the log, raw messages are removed from context. The `blockAfter` threshold forces a synchronous fallback if buffering can't keep up.

기본 설정:

* `observation.bufferTokens: 0.2`: 20%마다 버퍼링`messageTokens` (e.g. every ~6k tokens with a 30k threshold)
* `observation.bufferActivation: 0.8`: 활성화 시 남은 임계값의 20%만 유지되도록 충분한 메시지를 제거합니다.
* 버퍼링된 관찰에는 연속 힌트(`suggestedResponse`, `currentTask`) that survive activation to maintain conversational continuity
* `reflection.bufferActivation: 0.5`: 관찰 임계값의 50%에서 배경 반사를 시작합니다.
...
환경`bufferTokens: false` disables both observation and reflection async buffering. Observations and reflections will run synchronously when their thresholds are reached.

:::메모

비동기 버퍼링은 지원되지 않습니다.`scope: 'resource'` and is automatically disabled in resource scope.
...
두 완료 이벤트 모두 추출기 출력을 전달합니다.`data` payload. The extractor fields are:
...
두 추출기 필드는 모두 선택사항입니다. 완료에는 값, 실패, 둘 다 또는 둘 다 포함되지 않을 수 있습니다.`data-om-observation-end` reports synchronous completion. `data-om-buffering-end` reports completed background work whose buffered content still awaits activation, although extractor metadata is already persisted. `DataOmBufferingEndPart` carries the same extractor fields, and both types are exported from `@mastra/memory/processors`. See [Read extracted values from a stream](/docs/memory/observational-memory#read-extracted-values-from-a-stream) for a consumer example.
...
`buffered.reflection.inputObservationTokens`Reflector로 전송된 관측치의 크기입니다.`buffered.reflection.observationTokens` is the compressed result: the size of what will replace those observations when the reflection activates. A client can use these two values to show a compression ratio.
...
관찰자가 스레드 제목을 업데이트할 때 발생합니다. 다음 경우에만 방출됩니다.`observation.threadTitle` is enabled.
...
대부분의 사용자는`Memory` class above. Using `ObservationalMemory` directly is mainly useful for benchmarking, experimentation, or when you need to control processor ordering with other processors (like [guardrails](/docs/agents/guardrails)).

그만큼`ObservationalMemory` class is the engine; to attach it to an agent, wrap it in an `ObservationalMemoryProcessor`, which needs a `Memory` instance for loading and persisting messages. Note that `stores.memory` is typed as optional on storage adapters, so a non-null assertion (or a runtime check) is needed:
...
독립형`ObservationalMemory` class accepts all the same options as the `observationalMemory` config object above, plus the following:
...
어디`config` is an object with `upTo` keys that map token thresholds (numbers) to model targets.
...
* 임계값은 내부적으로 정렬되므로 구성 개체의 순서는 중요하지 않습니다.
* `inputTokens ≤ smallest threshold`→ 해당 임계값 Model을 사용합니다.
...
* OM은 Observer 또는 Reflector 호출에 대한 입력 토큰 수를 계산하고 일치하는 Model 계층을 직접 확인합니다.
