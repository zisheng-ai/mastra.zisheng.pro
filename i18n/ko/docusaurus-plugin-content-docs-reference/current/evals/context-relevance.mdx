...
참고: 둘 중 하나`context` or `contextExtractor` must be provided. If both are provided, `contextExtractor` takes precedence.
...
3. **적용되는 페널티**(다음을 통해 구성 가능`penalties` options):
   * **사용되지 않은 높은 관련성**: `unusedHighRelevanceContext` penalty per unused high-relevance context (default: 0.1)
   * **누락된 컨텍스트**: 최대`maxMissingContextPenalty` for identified missing information (default: 0.5)
...
* 응답에 실제로 사용된 컨텍스트는 무엇입니까?
* 사용되지 않은 관련성이 높은 컨텍스트에 적용되는 페널티(다음을 통해 구성 가능)`unusedHighRelevanceContext`)
* 응답을 개선할 수 있는 컨텍스트 누락(다음을 통해 처벌됨)`missingContextPerItem` up to `maxMissingContextPenalty`)

### 최적화 전략 {/* #optimization-strategies */}

결과를 사용하여 시스템을 개선하십시오.

* **관련 없는 컨텍스트 필터링**: 처리하기 전에 관련성이 낮은 부분이나 관련성이 없는 부분을 제거합니다.
* **컨텍스트 사용 보장**: 관련성이 높은 컨텍스트가 통합되었는지 확인하세요.
* **컨텍스트 공백 채우기**: 채점자가 확인한 누락정보 추가
* **밸런스 컨텍스트 크기**: 최고의 관련성을 위한 최적의 컨텍스트 양을 찾습니다.
* **페널티 감도 조정**: 조정하다`unusedHighRelevanceContext`, `missingContextPerItem`, and `maxMissingContextPenalty` based on your application's tolerance for unused or missing context
