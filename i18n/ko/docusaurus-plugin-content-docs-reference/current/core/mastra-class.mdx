...
생각하다`Mastra` as a top-level registry where you register agents, workflows, tools, and other components that need to be accessible throughout your application.
...
`notifications.dispatch.enabled`내부 디스패처 Workflow가 기본 크론으로 실행되도록 허용합니다.`*/1 * * * *`. The dispatcher reads due notification records from storage, groups summaries by `agentId`, `resourceId`, and `threadId`, Agent 스레드 런타임을 통해 신호를 내보냅니다. 이는 사용자가 직면하는 진입점이 아닙니다. 발송 일정(및 이를 뒷받침하는 Workflow 스케줄러)은 첫 번째 지연 또는 요약 알림에서 느리게 활성화되므로 알림을 연기하지 않는 앱은 스케줄러를 전혀 실행하지 않습니다.

## 생성자 매개변수 {/* #constructor-parameters */}

방문[Configuration reference](/reference/configuration) for detailed documentation on all available configuration options.
...
모든 고아를 다시 운전합니다.`running` durable-agent run across all registered durable agents. Called automatically on boot when `recovery.durableAgents` is `'auto'`. You can also call it directly for manual recovery or from a scheduled task.
