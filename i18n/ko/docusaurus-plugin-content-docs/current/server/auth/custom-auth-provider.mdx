...
확장`MastraAuthProvider` class and implement the required methods:
...
들어오는 토큰을 확인하고 유효한 경우 사용자 개체를 반환합니다. 또는`null` if authentication fails.
...
**보고**: 인증이 성공한 경우 사용자 개체 또는`null` if it fails.

토큰은 다음에서 자동으로 추출됩니다.`Authorization: Bearer <token>` header. If you need to access other headers or cookies, use the `request` parameter.
...
**보고**: `true` to allow access, `false` to deny (returns 403 Forbidden).
...
그만큼`@mastra/auth` package provides utilities for common token verification patterns:
...
참조[source code](https://github.com/mastra-ai/mastra/tree/main/auth) for implementation details.
