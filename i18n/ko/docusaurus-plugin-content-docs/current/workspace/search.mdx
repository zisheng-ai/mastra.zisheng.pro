...
사용자 정의 BM25 매개변수의 경우(`k1` is term frequency saturation, `b` is document length normalization):
...
공급자가 일괄 처리를 지원하는 경우(예: OpenAI의`embedMany`), pass an embedder that takes an array of texts and accepts many embeddings back in one call. To opt in, set a `batch: true` property on the function. Mastra checks for that property at runtime and switches to the batched path.
...
`Object.assign`을 추가합니다`batch` and `maxBatchSize` properties to the embedder function. Mastra reads them as metadata and never passes them to the provider.

단일 텍스트 삽입은 여전히 ​​작동합니다. 함수 서명`(text: string) => Promise<number[]>` is unchanged, so existing code keeps running without modification.
...
사용`workspace.index()` to add content to the search index programmatically. The file paths become document IDs. You can also pass metadata for each document.
...
구성`autoIndexPaths` to automatically index files when the workspace initializes. Each entry can be a directory path (indexed recursively) or a glob pattern for selective indexing.
...
언제`init()` is called, all matching files are read and indexed for search. The file path becomes the document ID.
...
사용`workspace.search()` to find relevant content. Results are ranked by relevance score.
...
작업 영역에서 검색을 구성하면 Agent는 콘텐츠 검색 및 인덱싱을 위한 Tool를 받습니다. 보다[workspace class reference](/reference/workspace/workspace-class#search-tools) for details.
