...
하위 Agent는 Tool 및 동적 구성에서 다음과 같은 항목을 읽습니다.`instructions: ({ requestContext }) => ...`. See [Request Context](/docs/server/request-context) for details. Values must be JSON-serializable to work with durable agents.
...
* 반품`{ feedback: '...' }`: Add feedback that gets saved to the parent agent's memory and is visible to subsequent iterations
...
기본적으로 하위 Agent는 상위 Agent로부터 전체 대화 컨텍스트를 받습니다. 사용`messageFilter` to control what messages are shared, for example, to remove sensitive data or limit context size.
...
콜백이 수신됩니다.`messages` (the full conversation history), `primitiveId` (the subagent ID), and `prompt` (the delegation prompt). Return the filtered array of messages.
...
애플리케이션 코드 및 UI 통합으로 계속 검사할 수 있습니다.`subAgentToolResults` and the rest of the raw delegation result in the tool result payload.

이렇게 하면 중첩된 Tool 인수나 출력을 상위 Agent의 다음 Model 호출로 다시 보내지 않고도 디버깅 및 표시 데이터를 계속 사용할 수 있습니다.

세트`includeSubAgentToolResultsInModelContext` to include the full subagent result, including nested tool results and subagent metadata, in the parent agent's model context.
...
Tool 승인은 위임 체인을 통해 전파됩니다. 하위 Agent가 다음과 같은 Tool를 사용하는 경우`requireApproval: true` or calls `suspend()`, the approval request surfaces in the parent agent's stream.
...
당신이`abortSignal` to the parent agent's [`stream()`](/reference/streaming/agents/stream) or [`generate()`](/reference/agents/generate) call, Mastra forwards that same signal to delegated subagents. Calling `AbortController.abort()` cancels in-flight subagent runs at their next step instead of letting them run to completion.
...
그것은로 작동합니다**LLM-as-judge** scorer. After each iteration, a separate grader model reviews the agent's output against the rubric. The loop ends when every required criterion passes. A failed criterion adds its feedback to the conversation so the agent can try again.
...
모 대리인의`instructions` should specify the available resources and when to use each one. They should also define coordination behavior and success criteria.

각 하위 Agent에는 명확한`description` that explains its purpose and return format, including when the parent agent should use it.
...
하위 Agent 호출은 Tool 호출로 전달되므로 다음과 같이 실행할 수 있습니다.[background tasks](/docs/long-running-agents/background-tasks). This is useful when one or more delegations are long-running and you don't want them to block the parent agent's response.

활성화[backgroundTasks manager](/reference/configuration#backgroundtasks) on the Mastra instance, then opt subagents in on the parent agent:
...
사용[`streamUntilIdle()`](/reference/streaming/agents/streamUntilIdle) instead of `stream()` so the stream stays open until the subagents complete and the parent agent has had a chance to respond to their results.

하위 Agent가 상위 Agent 아래에 나열되지 않은 경우`backgroundTasks.tools` but has its own background-eligible tools, the parent agent still dispatches the subagent as a background task and inherits its config. See [Inheriting from the subagent](/docs/long-running-agents/background-tasks#inheriting-from-the-subagent) for details.

## 하위 Agent 버전 관리 {/* #subagent-versioning */}

사용할 때[editor](/docs/editor/overview), you can control which stored version of each subagent the parent agent uses at runtime. Set version overrides on the Mastra instance or per invocation:
...
버전 재정의는 위임을 통해 자동으로 전파됩니다. 보다[Subagent versioning](/reference/editor/versioning#sub-agent-versioning) for details on resolution order and server API usage.
