...
필드 일치는 대소문자를 구분하지 않으며 구분 기호를 정규화합니다. 예를 들어,`api-key`, `api_key`, and `Api Key` are all treated as `apikey`.
...
민감한 필드가 감지되면 해당 값이 다음으로 대체됩니다.`[REDACTED]` by default. The filter handles nested objects, arrays, and circular references safely.
...
1. **대소문자를 구분하지 않음**: `APIKey`, `apikey`, and `ApiKey` are all matched
2. **구분 기호에 구애받지 않음**: `api-key`, `api_key`, and `apiKey` are treated identically
3. **정확한 일치**: 정규화 후 필드는 정확히 일치해야 합니다.
