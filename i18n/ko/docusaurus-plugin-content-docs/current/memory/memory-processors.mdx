...
Memory 프로세서는[processors](/docs/agents/processors) that operate specifically on memory-related messages and state.
...
1. 생성`MessageHistory` processor with `limit: 10`
...
Memory 프로세서를 수동으로 추가하는 경우`inputProcessors` or `outputProcessors`, Mastra **won't** automatically add it. This gives you full control over processor ordering:
...
이 주문은 다음과 같이 설계되었습니다.**safe by default**: if your output guardrail calls `abort()`, the memory processors never run and **no messages are saved**.
...
출력 가드레일 실행**before** memory processors save messages. If a guardrail aborts:
...
입력 가드레일 실행**after** memory processors load history. If a guardrail aborts:
...
맞춤형 프로세서를 생성할 때 입력을 변경하지 마십시오.`messages` array or its objects directly.
