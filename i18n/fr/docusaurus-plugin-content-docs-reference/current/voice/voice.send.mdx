...
description: "Documentation de the send() method available in real-time voice providers, which streams audio data for continuous processing."
packages:
  - "@mastra/node-audio"
  - "@mastra/node-speaker"
...
Renvoie a `Promise<void>` that resolves when the audio data has been accepted by the voice provider.
