---
title: "Référence : CompositeVoice | Voice"
description: "Documentation de the CompositeVoice class, which enables combining multiple voice providers for flexible text-to-speech and speech-to-text operations."
packages:
  - "@mastra/core"
  - "@mastra/voice-openai"
  - "@mastra/voice-playai"
...
- Renvoie a stream of audio data
...
- Renvoie either a string or a stream of transcribed text, depending on the provider

### `getSpeakers()`

Renvoie a list of available voices from the speaking provider, where each node contains:
...
- Renvoie voices from the speaking provider only
