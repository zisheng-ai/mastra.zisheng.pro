...
<PropertiesTable
  content={[
  {
    ...
    description:
      'Renvoie the model for the given input token count. Throws if inputTokens exceeds the largest configured threshold. When this happens during an OM run, callers receive a TripWire/empty-text outcome instead of a normal assistant response.',
  },
  {
    name: 'getThresholds',
    type: '() => number[]',
