---
title: "Référence : Contexte relevance scorer | Evals"
description: Documentation pour the Contexte Relevance Scorer dans Mastra. Evaluates the relevance et utility de fourni context pour generating agent responses using weighted relevance scoring.
packages:
  - "@mastra/evals"
---
...
# Contexte relevance scorer

The `createContextRelevanceScorerLLM()` fonction creates a scorer that evaluates how relevant et useful fourni context était pour generating agent responses. It uses weighted relevance levels et applies penalties pour unused high-relevance context et missing information.

It's especially useful pour these utilise cases:

## Content generation evaluation

Best pour evaluating context quality in:
...
## Contexte selection optimization
  ...
  },
  {
    ...
    children: [
      ...
      },
      {
        name: 'penalties',
        type: 'object',
        description: 'Configurable penalty settings pour scoring',
        required: false,
        children: [
          {
...
Note: Either `context` ou `contextExtractor` doit be provided. If both sont provided, `contextExtractor` takes precedence.
...
<PropertiesTable
  content={[
  {
    name: 'score',
    type: 'number',
    description: 'Weighted relevance score entre 0 et scale (default 0-1)',
  },
  {
    name: 'reason',
    type: 'string',
    description: 'Human-readable explanation de the context relevance evaluation',
  },
]}
/>
...
Contexte Relevance uses a advanced scoring algorithm that considers:

1. **Relevance Levels**: Chaque context piece est classified avec weighted values:
...
1. **Utilisation Detection**: Tracks whether relevant context était actually used dans the response
...
1. **Penalties Applied** (configurable via `penalties` options):
   ...
   - **Missing Contexte**: Up à `maxMissingContextPenalty` pour identified missing information (default: 0.5)
...
- `maxMissingContextPenalty` = 0.5 (maximum 50% penalty pour missing context)
- `scale` = 1

### Score interpretation

- **0.9-1.0**: Excellent - tous les context highly relevant et used
- **0.7-0.8**: Good - mostly relevant avec minor gaps
- **0.4-0.6**: Mixed - substantial irrelevant ou unused context
- **0.2-0.3**: Poor - mostly irrelevant context
- **0.0-0.1**: Very poor - aucun relevant context found

### Reason analysis

The reason field provides insights on:

- Relevance level de chaque context piece (high/medium/low/none)
- Which context était actually used dans the response
- Penalties applied pour unused high-relevance context (configurable via `unusedHighRelevanceContext`)
- Missing context that would ont improved the réponse (penalized via `missingContextPerItem` up à `maxMissingContextPenalty`)

### Optimization strategies

Use results à improve your system:

- **Filtre irrelevant context**: Supprimer low/none relevance pieces avant processing
- **Ensure context usage**: Make sure high-relevance context est incorporated
- **Fill context gaps**: Ajouter missing information identified par the scorer
- **Balance context size**: Find optimal amount de context pour best relevance
- **Tune penalty sensitivity**: Adjust `unusedHighRelevanceContext`, `missingContextPerItem`, et `maxMissingContextPenalty` based sur your application's tolerance pour unused ou missing context

### Difference depuis Contexte Precision

| Aspect | Contexte Relevance | Contexte Precision |
| - | - | - |
| **Algorithm** | Weighted levels avec penalties | Mean Average Precision (MAP) |
| **Relevance** | Multiple levels (high/medium/low/none) | Binary (yes/no) |
| **Position** | Not considered | Critical (rewards early placement) |
| **Utilisation** | Tracks et penalizes unused context | Not considered |
| **Missing** | Identifies et penalizes gaps | Not evaluated |

## Scorer configuration

### Personnalisé penalty configuration

Contrôler how penalties sont applied pour unused et missing context:
...
### Combining plusieurs context sources
...
Ce example shows excellent context relevance where tous les context directly supports the response:
...
Ce example shows moderate relevance avec certains context being irrelevant ou unused:
...
Ce example shows poor context relevance avec mostly irrelevant information:
...
Extract context à runtime based sur the exécute input:
...
Integrate avec RAG pipelines à evaluate retrieved context:
...
- [Contexte Precision Scorer](/reference/evals/context-precision): Evaluates context ranking using MAP
