---
title: "Référence : MCPServer | Tools and MCP"
description: Référence d’API de MCPServer — une classe permettant d’exposer les outils et les capacités de Mastra en tant que serveur Model Context Protocol.
packages:
  - "@mastra/core"
  - "@mastra/mcp"
---
...
<PropertiesTable
  ...
  },
  {
    ...
    isOptional: true,
    description:
      'Maps MCP transport auth data from `extra.authInfo` into the `user` value used by Mastra FGA checks. Use this when an OAuth-protected MCP server is registered on a Instance Mastra with an FGA provider.',
  },
  {
    name: 'fga',
...
### Bonnes pratiques for Prompt Handling
...
When the server is registered with a Instance Mastra, `toolActions.add()` and `toolActions.remove()` also update the Instance Mastra's tool registry, matching the automatic tool registration that happens at startup. Added tools become available through `mastra.listTools()` (keyed by the tool's intrinsic `id` when present), and removed tools are deleted from the registry.
...
- Renvoie `401 Unauthorized` with proper `WWW-Authenticate` headers when authentication is required
