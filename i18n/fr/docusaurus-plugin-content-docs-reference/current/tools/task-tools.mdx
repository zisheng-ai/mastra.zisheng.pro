---
title: "Référence : Task tools | Tools"
...
Renvoie a `TaskToolResult` with a human-readable `content` summary, the full `tasks` array with assigned IDs, and an `isError` flag.
...
- Renvoie an error with available task IDs when the ID isn't found.
...
Renvoie a `TaskCheckResult` with:
