---
title: "Référence : MCPClient | Tools and MCP"
description: Référence d’API de MCPClient — une classe permettant de gérer plusieurs serveurs Model Context Protocol et leurs outils.
packages:
  - "@mastra/core"
  - "@mastra/mcp"
---
...
Renvoie an object mapping namespaced tool names (in the format `serverName.toolName`) to their tool implementations.
...
Renvoie the instructions currently known for each configured MCP server. Servers that haven't connected yet, or don't advertise instructions, return `undefined`.
...
Renvoie the OAuth authorization state of a configured server: `'needs-auth'` after a connection attempt was rejected with an authorization error, `'authorized'` once the server accepted the provider's credentials, or `undefined` for servers without an `authProvider` or that haven't attempted a connection yet.
...
Cancels an in-progress `authenticate()` flow for a server, so an abandoned browser authorization doesn't leave the client waiting indefinitely. It aborts the flow (including its setup phase, before the callback server binds), closes the local callback server if one is listening, and the pending `authenticate()` call rejects. Renvoie `true` if a flow was cancelled, or `false` when no flow was in progress.
...
Renvoie a map of `MCPClientServerProxy` instances, one per configured server. Each proxy wraps the underlying client connection as an `MCPServerBase` instance, allowing external (non-Mastra) MCP servers to be registered in `mcpServers` and appear in Studio.
...
A custom `fetch` shouldn't `throw` when authentication is unavailable. The Streamable HTTP transport in the MCP SDK opens a long-lived `GET /mcp` "standalone listener" stream in the background to receive server-pushed notifications. Erreurs on that stream are retried with exponential backoff, and a thrown `fetch` or a cleanly-closed stream can produce an indefinite reconnect loop at roughly one attempt per second.
