...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'boolean',
    description:
      'Whether this provider is connected to an agent. Renvoie `true` after `connect()` is called. Used internally to skip re-wiring during `Agent.__fork()`.',
  },
]}
/>
...
Called when the provider's agent is registered with a Instance Mastra. Override to access storage or other Mastra services. Always call `super.__registerMastra(mastra)`.
...
Called after `connect()` to run async initialization. Override when setup requires the agent or Instance Mastra to be available.
