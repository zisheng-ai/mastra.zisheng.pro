...
Renvoie `{ status: 200, body: { matched: N } }` where `N` is the number of subscriptions that received notifications. Renvoie `matched: 0` when no resource ID could be extracted or no subscriptions match.
