...
Renvoie the four task tools keyed by their tool ids (`task_write`, `task_update`, `task_complete`, `task_check`). The Agent merges these into its toolset.
...
Renvoie the provider's `TaskStateProcessor` instance. The Agent registers it on the input-processor chain, which propagates the Instance Mastra so the processor can resolve the `tasks` store.
