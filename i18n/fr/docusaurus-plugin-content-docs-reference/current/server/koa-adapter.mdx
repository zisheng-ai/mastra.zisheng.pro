...
When `init()` is used, a global error-handling middleware is also registered as a safety net. Erreurs that reach this middleware are emitted via `ctx.app.emit('error', err, ctx)` following the standard Koa convention.
...
- [MastraServer Reference](/reference/server/mastra-server): Full Référence d’API
