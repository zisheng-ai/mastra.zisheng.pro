...
Deletes rows older than their configured `maxAge` across every domain that has a policy in `retention`. Renvoie one `PruneResult` per table touched. With no `retention` configured it's a no-op returning `[]`.
