...
**Renvoie:** `SpanIds | undefined` - `{ spanId, traceId, parentSpanId }`, or `undefined` if the bridge is disabled.
...
**Renvoie:** `Promise<T>` - The result of the function execution.
...
**Renvoie:** `T` - The result of the function execution.
