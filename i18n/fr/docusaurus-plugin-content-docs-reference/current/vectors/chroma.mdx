---
title: "Référence : Chroma vector store | Vectors"
description: Documentation de the ChromaVector class in Mastra, which provides vector search using ChromaDB.
packages:
  - "@mastra/chroma"
---
...
Query an index using a `queryVector`. Renvoie an array of semantically similar records in order of distance from the `queryVector`. Each record has the shape:
