...
description: "Documentation de the PlatformFilesystem provider for bucket-backed storage on Mastra Platform."
packages:
...
Voir aussi providers: [`S3Filesystem`](/reference/workspace/s3-filesystem) for direct S3 access, [`LocalFilesystem`](/reference/workspace/local-filesystem) for local directories.
