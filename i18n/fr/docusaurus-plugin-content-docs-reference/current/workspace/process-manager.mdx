...
Spawn a background process. Renvoie a `ProcessHandle` immediately without waiting for the process to finish.
...
List all tracked processes. Renvoie info about each process including PID, running state, and exit code.
...
Get a handle to a process by PID. Renvoie `undefined` if the process isn't found or has already been dismissed.
...
Kill a process by PID. Waits for the process to terminate before returning. Renvoie `true` if the process was killed, `false` if it wasn't found.
...
Kill the process. Renvoie `true` if the process was killed, `false` if it had already exited.
