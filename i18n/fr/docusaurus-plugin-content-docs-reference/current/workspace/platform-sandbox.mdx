...
description: "Documentation de the PlatformSandbox provider for executing commands in a Mastra Platform environment."
packages:
...
Voir aussi providers: [`RailwaySandbox`](/reference/workspace/railway-sandbox) for self-hosted Railway sandboxes, [`LocalSandbox`](/reference/workspace/local-sandbox) for local sandboxes.
