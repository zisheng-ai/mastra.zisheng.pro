---
title: "Référence : Workspace class | Workspace"
description: "Documentation de the Workspace class, the central API for filesystem access, sandbox execution, and search in Mastra."
packages:
  - "@mastra/core"
---
...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'The static filesystem provider. Renvoie `undefined` when a resolver function is configured — use `hasFilesystemConfig()` to check availability.',
  },
  {
    name: 'sandbox',
    type: 'WorkspaceSandbox | undefined',
    description:
      'The static sandbox provider. Renvoie `undefined` when a resolver function is configured — use `hasSandboxConfig()` to check availability.',
  },
  {
    name: 'skills',
...
Renvoie combined instructions from the filesystem and sandbox providers. Injected into the agent's system message to help it understand the execution context.
...
Renvoie combined workspace instructions. Use this when the workspace uses resolver-backed providers. A runtime-defined filesystem is resolved per request; a runtime-defined sandbox contributes stable placeholder text unless `instructions.dynamicSandbox` is set to `'resolve'`.
...
Resolve the filesystem for a request context. When a resolver function is configured, calls it with the provided `requestContext`. When a static filesystem is configured, returns it directly. Renvoie `undefined` if no filesystem is configured.
...
Resolve the sandbox for a request context. When a resolver function is configured, calls it with the provided `requestContext`. When a static sandbox is configured, returns it directly. Renvoie `undefined` if no sandbox is configured.
...
| Tool | Description |
| - | - |
| `mastra_workspace_execute_command` | Execute a shell command. Renvoie stdout, stderr, and exit code. When the sandbox has a process manager, accepts `background: true` to spawn a long-running process and return a PID. |
...
| `mastra_workspace_kill_process` | Kill a background process by PID. Renvoie the last 50 lines of output. Only available when the sandbox has a process manager. |
...
| Tool | Description |
| - | - |
| `mastra_workspace_search` | Search indexed content using keyword (BM25), semantic (vector), or hybrid search. Renvoie ranked results with scores. |
...
| Tool | Description |
| - | - |
| `skill` | Activate a skill by name or path. Renvoie the skill's full instructions, references, scripts, and assets. |
