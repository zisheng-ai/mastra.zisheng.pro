...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Callback invoked when a cost violation is detected, regardless of strategy. Part of the generalized Processor interface. Use for side effects like alerting, logging to external systems, or emailing users. Erreurs thrown by this callback are silently caught.',
    isOptional: true,
  },
  {
...
All scopes require observability storage with `getMetricAggregate` support. If the Instance Mastra doesn't have observability storage configured, an error is thrown at registration time.
