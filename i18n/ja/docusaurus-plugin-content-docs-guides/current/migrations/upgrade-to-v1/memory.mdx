...
### Semantic Recall と直近メッセージのデフォルト設定 {/* #default-settings-for-semantic-recall-and-last-messages */}
...
### デフォルトの Memory スコープを `thread` から `resource` へ変更 {/* #default-memory-scope-from-thread-to-resource */}
...
### Thread タイトル生成の設定場所 {/* #thread-title-generation-location */}
...
### Semantic Recall のデフォルト設定を最適化 {/* #semantic-recall-default-settings-optimization */}
...
### `memory.readOnly` を `memory.options.readOnly` へ移動 {/* #memoryreadonly-moved-to-memoryoptionsreadonly */}
...
### `Memory.query()` を `Memory.recall()` へ改名 {/* #memoryquery-renamed-to-memoryrecall */}
...
### `Memory.recall()` のパラメーター変更 {/* #memoryrecall-parameter-changes */}
...
### `MastraMessageV2` 型を `MastraDBMessage` へ改名 {/* #mastramessagev2-type-renamed-to-mastradbmessage */}
...
### Working Memory の `text-stream` モード {/* #working-memory-text-stream-mode */}
...
### `Memory.rememberMessages()` メソッド {/* #memoryremembermessages-method */}
...
### Memory メソッドの `format` パラメーター {/* #format-parameter-from-memory-methods */}
...
### `MastraMessageV3` 型 {/* #mastramessagev3-type */}
...
### Memory コンストラクターの `processors` 設定 {/* #processors-config-from-memory-constructor */}
