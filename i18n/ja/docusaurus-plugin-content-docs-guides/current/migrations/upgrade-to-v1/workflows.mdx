...
### `getWorkflows` から `listWorkflows` へ {/* #getworkflows-to-listworkflows */}
...
### Step コンテキストの `RuntimeContext` から `RequestContext` へ {/* #runtimecontext-to-requestcontext-in-step-context */}
...
### `createRunAsync` から `createRun` へ {/* #createrunasync-to-createrun */}
...
### `runCount` から `retryCount` へ（非推奨） {/* #runcount-to-retrycount-deprecated */}
...
### `getInitData` が unknown を返すように変更 {/* #getinitdata-returns-unknown */}
...
### 入力をデフォルトで検証 {/* #inputs-are-validated-by-default */}
...
### Step の `suspendPayload` 検証 {/* #step-suspendpayload-validation */}
...
### Branch の結果フィールドが任意に {/* #branch-result-fields-are-now-optional */}
...
### `Run.start()` と `Run.timeTravel()` の `writableStream` から `outputWriter` へ {/* #writablestream-to-outputwriter-in-runstart--runtimetravel */}
...
### `setState()` を非同期化し、渡すデータを検証 {/* #setstate-is-now-async-and-the-data-passed-is-validated */}
...
### Step 条件関数のパラメーターから `suspend()` と `setState()` を削除 {/* #suspend-and-setstate-arent-available-in-step-condition-functions-parameters */}
...
### レガシー Workflow の export {/* #legacy-workflows-export */}
...
### `WorkflowRunOutput` の `pipeThrough` メソッドと `pipeTo` メソッド {/* #pipethrough-and-pipeto-methods-from-workflowrunoutput */}
...
### Watch Event API {/* #watch-events-api */}
