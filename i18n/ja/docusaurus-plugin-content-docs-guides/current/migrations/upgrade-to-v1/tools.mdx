...
### `createTool` の execute シグネチャを `(inputData, context)` 形式へ変更 {/* #createtool-execute-signature-to-inputdata-context-format */}
...
### `createTool` のコンテキストプロパティの構成 {/* #createtool-context-properties-organization */}
...
### `outputSchema` による Tool 出力の検証 {/* #tool-output-validation-with-outputschema */}
...
### `tool.execute` の戻り値の型に `ValidationError` を追加 {/* #toolexecute-return-type-includes-validationerror */}
...
### Tool の直接実行 {/* #direct-tool-execution */}
