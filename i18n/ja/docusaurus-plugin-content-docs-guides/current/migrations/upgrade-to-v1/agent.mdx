...
### `getAgents` から `listAgents` へ {/* #getagents-to-listagents */}
...
### `RuntimeContext` から `RequestContext` へ {/* #runtimecontext-to-requestcontext */}
...
### プロパティへの直接アクセスから getter メソッドへ {/* #direct-property-access-to-getter-methods */}
...
### Voice メソッドを `agent.voice` 名前空間へ移動 {/* #voice-methods-moved-to-agentvoice-namespace */}
...
### `agent.fetchMemory()` から `(await agent.getMemory()).recall()` へ {/* #agentfetchmemory-to-await-agentgetmemoryrecall */}
...
### Processor メソッド名を `get*` から `list*` へ変更 {/* #processor-method-names-from-get-to-list */}
...
### Zod v3 と v4 の構造化出力スキーマを引き続きサポート {/* #zod-v3-and-v4-structured-output-schemas-remain-supported */}
...
### AI SDK バージョン別にデフォルトオプションメソッドを改名 {/* #default-options-method-renames-for-ai-sdk-versions */}
...
### `modelSettings.abortSignal` からトップレベルの `abortSignal` へ {/* #modelsettingsabortsignal-to-top-level-abortsignal */}
...
### `output` から `structuredOutput.schema` へ {/* #output-to-structuredoutputschema */}
...
### Agent の `id` フィールドが必須に {/* #agent-id-field-is-now-required */}
...
### Stream API のレスポンスで機密データを自動的にマスク {/* #stream-api-responses-now-redact-sensitive-data */}
...
### `generateVNext` メソッドと `streamVNext` メソッド {/* #generatevnext-and-streamvnext-methods */}
...
### `stream()` と `generate()` の `format` パラメーター {/* #format-parameter-from-stream-and-generate */}
...
### `agent.toStep()` メソッド {/* #agenttostep-method */}
...
### Agent の `TMetrics` ジェネリックパラメーター {/* #tmetrics-generic-parameter-from-agent */}
...
### Tripwire のレスポンス形式を変更 {/* #tripwire-response-format-changed */}
...
### `prepareStep` のメッセージ形式 {/* #preparestep-messages-format */}
