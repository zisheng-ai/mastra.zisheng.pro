...
## Adapter ごとのオプション {/* #per-adapter-options */}
...
<PropertiesTable
  ...
  },
  {
    ...
    description: 'チャットでのエラー表示方法を上書きします。生のエラーを公開せず、ユーザー向けのメッセージを返します。',
  },
  {
    name: 'formatToolCall',
...
表示を完全にカスタマイズするには、`toolDisplay` に関数を渡します。関数は `ToolDisplayEvent`（`running`／`result`／`error`／`approval`）と `ToolDisplayContext`（`{ mode, platform }`）を受け取ります。個別の投稿／編集には `{ kind: 'post',
message }` for a discrete post/edit, `{ kind: 'stream', chunk }` を返し、
...
## カスタム入力中ステータス {/* #custom-typing-status */}
...
## Resource ID の解決 {/* #resource-id-resolution */}
...
## Thread ID の解決 {/* #thread-id-resolution */}
