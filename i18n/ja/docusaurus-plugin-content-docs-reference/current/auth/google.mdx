    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            name: 'privateKey',
            type: 'string',
            description: 'PEM 形式でエンコードされた秘密鍵。.env ファイルのエスケープされた `\\n` 値もサポートされます。',
          },
          {
            name: 'privateKeyId',
            type: 'string',
...
`user.groups` がすでに配列である場合、`MastraRBACGoogle` はその値を使用し、Directory API を呼び出しません。空の `groups` 配列は、ユーザーに Google グループロールがないことを意味し、`_default` が設定されていれば、その権限に解決されます。
