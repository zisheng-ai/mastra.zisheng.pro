...
### 一時的な Sandbox {/* #ephemeral-sandbox */}
...
### Filesystem のマウント {/* #filesystem-mounting */}

S3 または GCS バケットを Sandbox 内のローカルディレクトリとしてマウントします。

#### Workspace の mounts 設定を使用する {/* #via-workspace-mounts-config */}
...
#### `sandbox.mount()` を使用する {/* #via-sandboxmount */}
...
#### S3 互換（Cloudflare R2、MinIO） {/* #s3-compatible-cloudflare-r2-minio */}
...
### S3 の環境変数 {/* #s3-environment-variables */}
...
### GCS の環境変数 {/* #gcs-environment-variables */}
...
### スナップショットでコールドスタートの遅延を短縮する {/* #reducing-cold-start-latency-with-a-snapshot */}
...
## SDK への直接アクセス {/* #direct-sdk-access */}
...
## Sandbox の作成モード {/* #sandbox-creation-modes */}
