...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'Handler',
    description:
      'Hono Context を受け取るルートハンドラー関数。`handler` と `createHandler` のいずれか一方を使用し、両方は使用しないでください。',
    isOptional: true,
  },
  {
    name: 'createHandler',
...
### パスパラメーターを持つルート {/* #route-with-path-parameters */}
...
### ボディを持つ POST ルート {/* #post-route-with-body */}
...
### ミドルウェアを持つルート {/* #route-with-middleware */}
...
### OpenAPI ドキュメントを持つルート {/* #route-with-openapi-documentation */}
...
### `createHandler()` の使用 {/* #using-createhandler */}
