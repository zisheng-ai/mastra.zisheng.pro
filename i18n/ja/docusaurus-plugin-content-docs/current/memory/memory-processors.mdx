...
# Memory Processor {/* #memory-processors */}
...
## 組み込みの Memory Processor {/* #built-in-memory-processors */}
...
## 手動制御と重複排除 {/* #manual-control-and-deduplication */}
...
## Processor の実行順序 {/* #processor-execution-order */}
...
## ガードレールと Memory {/* #guardrails-and-memory */}

デフォルトの実行順序により、ガードレールは安全に動作します。

### 出力ガードレール（推奨） {/* #output-guardrails-recommended */}
...
## 大きな添付ファイルを処理する {/* #handling-large-attachments */}
...
- [Processor](/docs/agents/processors)：Processor の一般的な概念とカスタム Processor の作成
- [ガードレール](/docs/agents/guardrails)：セキュリティと検証のための Processor
- [Memory の概要](/docs/memory/overview)：Memory の種類と設定
