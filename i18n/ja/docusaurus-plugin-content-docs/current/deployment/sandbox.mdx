...
プロバイダーの開発者は、[`WorkspaceSandbox`](/reference/workspace/sandbox) にオプションの `networking` capability を実装することで、サポートを追加できます。
...
Daytona Sandbox を停止すると、ファイルシステムは保持されますが、実行中のプロセスは保持されません。そのため、復帰時の動作は Vercel と同様です。resolver は `wake: true` の場合にサーバーを再起動します。
...
TLS ハンドシェイク中に `Connection reset by peer` が発生する場合、Agent ではなくフィルタリングが原因です。送信先を許可するよう Daytona サポートに依頼してください。制限付きの Daytona tier ではクラウドストレージのエンドポイントもブロックされ、その場合はマウント用ヘルパーが専用のエラーを報告します。
...
Sandbox の停止と再開に伴い、Sandbox URL が変わる場合があります。URL は接続のための情報として扱い、Sandbox の ID（たとえば `sandboxName`）を安定したハンドルとして扱ってください。以下のルーティング tier は URL の切り替わりに対応します。
