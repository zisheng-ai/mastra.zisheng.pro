...
## Agent の承認を使用する場面 {/* #when-to-use-agent-approval */}
...
## 承認の仕組み {/* #how-approval-works */}

Mastra には、Tool の呼び出しを一時停止する方法が 2 つあります。**実行前の承認**と**実行中の一時停止**です。

### 実行前の承認 {/* #pre-execution-approval */}

実行前の承認では、Tool の `execute` 関数が実行される\_前\_に呼び出しを一時停止します。呼び出す Tool と引数は引き続き LLM が決定しますが、明示的に承認するまで `execute` は実行されません。

次のフラグは OR 条件で動作します。\_どちらか\_が `true` なら呼び出しが一時停止します。
...
#### 関数による条件付き承認 {/* #conditional-approval-with-a-function */}
...
#### 承認を正確な Tool 引数に紐付ける {/* #bind-approval-to-the-exact-tool-arguments */}
...
### `suspend()` による実行中の一時停止 {/* #runtime-suspension-with-suspend */}

Tool は `execute` 関数の\_実行中\_に `suspend()` を呼び出して一時停止することもできます。これは、Tool が実行を開始した後、完了前に追加のユーザー入力や確認が必要だと判明した場合に役立ちます。
...
## `generate()` での Tool の承認 {/* #tool-approval-with-generate */}
...
### stream と generate の比較 {/* #stream-vs-generate-comparison */}
...
### `suspend()` による承認 {/* #approval-using-suspend */}
...
## Tool の自動再開 {/* #automatic-tool-resumption */}
...
### 手動再開と自動再開 {/* #manual-vs-automatic-resumption */}
...
## 再起動後に再開する {/* #resuming-after-a-restart */}
...
## Supervisor Agent での Tool の承認 {/* #tool-approval-supervisor-agents */}
...
### Supervisor Agent で承認または拒否する {/* #approve-and-decline-in-supervisor-agents */}
...
### Supervisor Agent で `suspend()` を使用する {/* #use-suspend-in-supervisor-agents */}
...
### `generate()` を使用した Supervisor の承認 {/* #supervisor-approval-with-generate */}
