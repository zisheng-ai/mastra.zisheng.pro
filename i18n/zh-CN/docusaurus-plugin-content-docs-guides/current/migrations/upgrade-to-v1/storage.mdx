...
### 重命名 scorer 表列 {/* #scorers-table-column-rename */}
...
### 重复 span 迁移 {/* #duplicate-spans-migration */}
...
#### 方案 1：使用 CLI（推荐） {/* #option-1-use-the-cli-recommended */}
...
#### 方案 2：手动执行 SQL（PostgreSQL） {/* #option-2-manual-sql-postgresql */}
...
#### 方案 3：手动迁移（其他数据库） {/* #option-3-manual-migration-other-databases */}
...
### `MastraCompositeStore` 中的 Storage 组合 {/* #storage-composition-in-mastracompositestore */}
...
### `MastraStorage` 重命名为 `MastraCompositeStore` {/* #mastrastorage-renamed-to-mastracompositestore */}
...
### Storage 实例必须提供 `id` 属性 {/* #required-id-property-for-storage-instances */}
...
### 分页从 `offset/limit` 改为 `page/perPage` {/* #pagination-from-offsetlimit-to-pageperpage */}
...
### `getMessagesPaginated` 改为 `listMessages` {/* #getmessagespaginated-to-listmessages */}
...
### 通过 `getStore()` 访问特定领域的 Storage {/* #domain-specific-storage-access-via-getstore */}
...
### `getThreadsByResourceId` 改为 `listThreads` {/* #getthreadsbyresourceid-to-listthreads */}
...
### `getWorkflowRuns` 改为 `listWorkflowRuns` {/* #getworkflowruns-to-listworkflowruns */}
...
### `getMessagesById` 改为 `listMessagesById` {/* #getmessagesbyid-to-listmessagesbyid */}
...
### Storage `getMessages` 和 `saveMessages` 签名 {/* #storage-getmessages-and-savemessages-signatures */}
...
### Vector Store API 从位置参数改为命名参数 {/* #vector-store-api-from-positional-to-named-arguments */}
...
### Vector Store 方法重命名 {/* #vector-store-method-renames */}
...
### PGVector 构造函数从连接字符串改为对象 {/* #pgvector-constructor-from-connection-string-to-object */}
...
### PGVector `defineIndex` 改为 `buildIndex` {/* #pgvector-defineindex-to-buildindex */}
...
### `PostgresStore`：`schema` 改为 `schemaName` {/* #postgresstore-schema-to-schemaname */}
...
### 评分 Storage 方法改为 `listScoresBy*` 模式 {/* #score-storage-methods-to-listscoresby-pattern */}
...
### 非分页 Storage 函数 {/* #non-paginated-storage-functions */}
...
### Evals 测试工具函数 {/* #evals-test-utilities */}
...
### MSSQL Storage 中的 TABLE\_EVALS {/* #table_evals-from-mssql-storage */}
