...
### `getAgents` 改为 `listAgents` {/* #getagents-to-listagents */}
...
### `RuntimeContext` 改为 `RequestContext` {/* #runtimecontext-to-requestcontext */}
...
### 直接属性访问改为 getter 方法 {/* #direct-property-access-to-getter-methods */}
...
### Voice 方法迁移到 `agent.voice` 命名空间 {/* #voice-methods-moved-to-agentvoice-namespace */}
...
### `agent.fetchMemory()` to `(await agent.getMemory()).recall()` {/* #agentfetchmemory-to-await-agentgetmemoryrecall */}
...
### Processor 方法名从 `get*` 改为 `list*` {/* #processor-method-names-from-get-to-list */}
...
### 继续支持 Zod v3 和 v4 结构化输出 Schema {/* #zod-v3-and-v4-structured-output-schemas-remain-supported */}
...
### 针对 AI SDK 版本重命名默认选项方法 {/* #default-options-method-renames-for-ai-sdk-versions */}
...
### `modelSettings.abortSignal` 改为顶层 `abortSignal` {/* #modelsettingsabortsignal-to-top-level-abortsignal */}
...
### `output` 改为 `structuredOutput.schema` {/* #output-to-structuredoutputschema */}
...
### Agent 现在必须提供 `id` 字段 {/* #agent-id-field-is-now-required */}
...
### Stream API 响应现在会遮盖敏感数据 {/* #stream-api-responses-now-redact-sensitive-data */}
...
### `stream()` 和 `generate()` 中的 `format` 参数 {/* #format-parameter-from-stream-and-generate */}
...
### Agent 中的 `TMetrics` 泛型参数 {/* #tmetrics-generic-parameter-from-agent */}
...
### Tripwire 响应格式变更 {/* #tripwire-response-format-changed */}
...
### `prepareStep` 消息格式 {/* #preparestep-messages-format */}
...
### `threadId` 和 `resourceId` 改为 `memory` 选项 {/* #threadid-and-resourceid-to-memory-option */}
