...
### `createTool` 的 execute 签名改为 `(inputData, context)` 格式 {/* #createtool-execute-signature-to-inputdata-context-format */}
...
### `createTool` 上下文属性的组织方式 {/* #createtool-context-properties-organization */}
...
### 使用 `outputSchema` 验证 Tool 输出 {/* #tool-output-validation-with-outputschema */}
...
### `tool.execute` 返回类型包含 `ValidationError` {/* #toolexecute-return-type-includes-validationerror */}
...
### 直接执行 Tool {/* #direct-tool-execution */}
