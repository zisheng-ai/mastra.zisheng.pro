...
### `messages` 现在与 `@mastra/core/agent` 语法一致 {/* #messages-is-now-identical-to-mastracoreagent-syntax */}
...
### Client SDK 类型从 `Get*` 改为 `List*` {/* #client-sdk-types-from-get-to-list */}
...
### 分页参数从 `offset/limit` 改为 `page/perPage` {/* #pagination-parameters-from-offsetlimit-to-pageperpage */}
...
### `getMemoryThread` 参数结构 {/* #getmemorythread-parameter-structure */}
...
### 用于 Workflow run 的统一 `runById` API {/* #unified-runbyid-api-for-workflow-runs */}
...
### `runExecutionResult` 方法和 `GetWorkflowRunExecutionResultResponse` 类型 {/* #runexecutionresult-method-and-getworkflowrunexecutionresultresponse-type */}
...
### `toAISdkFormat` 函数 {/* #toaisdkformat-function */}
...
### Watch 相关类型 {/* #watch-related-types */}
...
### 不能直接在 Workflow 实例上调用 run 相关方法 {/* #run-related-methods-cant-be-called-directly-on-workflow-instance */}
...
### 已弃用的流式端点 {/* #deprecated-stream-endpoints */}
...
### Network Memory API 端点 {/* #network-memory-api-endpoints */}
...
### Evals 相关的 Client SDK 类型 {/* #eval-related-client-sdk-types */}
