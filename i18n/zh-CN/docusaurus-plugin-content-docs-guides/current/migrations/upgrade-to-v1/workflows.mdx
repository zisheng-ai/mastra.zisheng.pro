...
### `getWorkflows` 改为 `listWorkflows` {/* #getworkflows-to-listworkflows */}
...
### 步骤上下文中的 `RuntimeContext` 改为 `RequestContext` {/* #runtimecontext-to-requestcontext-in-step-context */}
...
### `createRunAsync` 改为 `createRun` {/* #createrunasync-to-createrun */}
...
### `runCount` 改为 `retryCount`（已弃用） {/* #runcount-to-retrycount-deprecated */}
...
### `getInitData` 返回 unknown {/* #getinitdata-returns-unknown */}
...
### 默认验证输入 {/* #inputs-are-validated-by-default */}
...
### 步骤 `suspendPayload` 验证 {/* #step-suspendpayload-validation */}
...
### 分支结果字段现在是可选的 {/* #branch-result-fields-are-now-optional */}
...
### `Run.start()` 和 `Run.timeTravel()` 中的 `writableStream` 改为 `outputWriter` {/* #writablestream-to-outputwriter-in-runstart--runtimetravel */}
...
### `setState()` 现在是异步函数，并会验证传入的数据 {/* #setstate-is-now-async-and-the-data-passed-is-validated */}
...
### 步骤条件函数参数中不再提供 `suspend()` 和 `setState()` {/* #suspend-and-setstate-arent-available-in-step-condition-functions-parameters */}
...
### 旧版 Workflow 导出 {/* #legacy-workflows-export */}
...
### `WorkflowRunOutput` 中的 `pipeThrough` 和 `pipeTo` 方法 {/* #pipethrough-and-pipeto-methods-from-workflowrunoutput */}
