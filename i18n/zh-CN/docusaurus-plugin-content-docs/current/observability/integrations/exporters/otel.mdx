...
### 对于 HTTP/Protobuf Provider（SigNoz、New Relic、Laminar、MLflow、Latitude、telemetry.dev） {/* #for-httpprotobuf-providers-signoz-new-relic-laminar-mlflow-latitude-telemetrydev */}

```bash npm2yarn
npm install @mastra/otel-exporter@latest @opentelemetry/exporter-trace-otlp-proto
```

### 对于 `gRPC` Provider（Dash0、Datadog） {/* #for-grpc-providers-dash0-datadog */}

```bash npm2yarn
npm install @mastra/otel-exporter@latest @opentelemetry/exporter-trace-otlp-grpc @grpc/grpc-js
```

### 对于 HTTP/JSON Provider（Traceloop） {/* #for-httpjson-providers-traceloop */}
...
### 自定义/通用 OTEL endpoint {/* #customgeneric-otel-endpoints */}
...
### 缺少依赖错误 {/* #missing-dependency-error */}
