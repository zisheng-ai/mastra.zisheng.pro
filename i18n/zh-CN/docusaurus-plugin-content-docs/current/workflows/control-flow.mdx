...
## 使用 `.then()` 链接步骤 {/* #chaining-steps-with-then */}
...
## 使用 `.parallel()` 同时运行步骤 {/* #simultaneous-steps-with-parallel */}
...
## 使用 `.branch()` 实现条件逻辑 {/* #conditional-logic-with-branch */}
...
### Parallel 和 Branch 输出 {/* #parallel-and-branch-outputs */}
...
### 使用 `.dountil()` 循环 {/* #looping-with-dountil */}
...
### 使用 `.dowhile()` 循环 {/* #looping-with-dowhile */}
...
### 使用 `.foreach()` 循环 {/* #looping-with-foreach */}
...
#### 在 `.foreach()` 后聚合结果 {/* #aggregating-results-after-foreach */}
...
#### 链接多个 `.foreach()` 调用 {/* #chaining-multiple-foreach-calls */}
...
#### foreach 内的嵌套 Workflow {/* #nested-workflows-inside-foreach */}
...
## 选择正确模式 {/* #choosing-the-right-pattern */}
...
### `.parallel()` 与 `.foreach()` {/* #parallel-vs-foreach */}
...
### 何时使用嵌套 Workflow {/* #when-to-use-nested-workflows */}
...
### 同步：下一步骤何时运行？ {/* #synchronization-when-does-the-next-step-run */}
