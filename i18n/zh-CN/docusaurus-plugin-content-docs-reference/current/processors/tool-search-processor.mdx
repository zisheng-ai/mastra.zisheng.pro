...
<PropertiesTable
  ...
  },
  {
    ...
    description: '处理每个步骤，将搜索或加载元 Tool 以及此前加载的所有 Tool 注入 Agent 的 Tool 集合。',
    isOptional: false,
  },
]}
/>
...
### 状态检查（旧版 `'in-memory'` 存储） {/* #state-inspection-legacy-in-memory-store */}
...
## 请求感知过滤 {/* #request-aware-filtering */}
...
## 搜索为请求解析的 Tool {/* #searching-request-resolved-tools */}
...
## 使用 `autoLoad` 进行单步骤发现 {/* #single-step-discovery-with-autoload */}
...
## 已加载 Tool 的存储 {/* #loaded-tool-storage */}

`storage` 选项控制在哪里跟踪已加载的 Tool 集合。默认值为 `'in-memory'`。`'context'` 存储需选择启用。

### `'in-memory'`（默认） {/* #in-memory-default */}
...
## 与其他 Processor 结合使用 {/* #combining-with-other-processors */}
