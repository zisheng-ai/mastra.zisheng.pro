...
## Processor 方法的运行时机 {/* #when-processor-methods-run */}
...
## 消息参数 {/* #message-arguments */}

大多数 Processor 方法都会同时接收 `messages` 和 `messageList`。它们指向同一个底层对话，但提供不同的访问方式。

### `messages` 与 `messageList` {/* #messages-vs-messagelist */}
...
### 从消息中读取文本 {/* #reading-text-from-a-message */}
...
#### agentic loop 中的执行顺序 {/* #execution-order-in-the-agentic-loop */}
...
#### Processor 链式处理 {/* #processor-chaining */}
...
#### 系统消息隔离 {/* #system-message-isolation */}
...
#### 示例：自定义错误恢复 {/* #example-custom-error-recovery */}
...
#### 示例：支持重试的质量 guardrail {/* #example-quality-guardrail-with-retry */}
...
#### 示例：遮盖敏感字段 {/* #example-redact-sensitive-fields */}
...
#### 示例：阻止 Tool 输出中的 prompt injection {/* #example-block-prompt-injection-in-tool-output */}
...
### 基础 input Processor {/* #basic-input-processor */}
...
### 使用 `processInputStep` 的逐步骤 Processor {/* #per-step-processor-with-processinputstep */}
...
### 使用 `processInputStep` 的消息转换 Processor {/* #message-transformer-with-processinputstep */}
...
### 混合 Processor（输入和输出） {/* #hybrid-processor-input-and-output */}
...
### 使用状态的流累加 Processor {/* #stream-accumulator-with-state */}
...
## 中止与 tripwire 数据块 {/* #aborting-and-tripwire-chunks */}
...
## 发出自定义数据块 {/* #emitting-custom-data-chunks */}
...
## 在 Agent 上配置 Processor {/* #configuring-processors-on-an-agent */}
