...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    description:
      "部署环境（可按环境筛选问题和告警）。未设置时使用 SENTRY_ENVIRONMENT 环境变量。默认：'production'。",
    required: false,
  },
  {
  ...
  },
  {
    ...
    type: 'string',
    description:
      '已部署代码的版本（有助于识别回归问题和跟踪部署）。未设置时使用 SENTRY_RELEASE 环境变量。',
    required: false,
  },
  {
    name: 'options',
    ...
    description:
      '其他 Sentry SDK 选项（integrations、beforeSend 等）。可用选项请参阅 Sentry SDK 文档。',
    required: false,
  },
]}
/>
