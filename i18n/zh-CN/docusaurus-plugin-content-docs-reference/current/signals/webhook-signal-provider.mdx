...
<PropertiesTable
  content={[
  { name: 'id', type: 'string', description: 'Provider 实例的唯一标识符。', isOptional: true, defaultValue: "'webhook-signals'" },
  { name: 'name', type: 'string', description: '人类可读的名称。', isOptional: true, defaultValue: "'Webhook Signals'" },
  { name: 'extractResourceId', type: '(payload: unknown) => string | string[] | undefined', description: '从 webhook payload 提取外部资源 ID。返回 `undefined` 可跳过该事件。可返回数组以匹配多个资源。', isOptional: true, defaultValue: '如果存在则返回 `payload.resource` 或 `payload.externalResourceId`' },
  { name: 'buildNotification', type: '(payload: unknown, subscription: SignalSubscription) => SendNotificationSignalInput', description: '根据 webhook payload 和匹配的订阅构建通知对象。', isOptional: true, defaultValue: "返回 `{ source: id, kind: 'webhook-event', summary, payload }`" },
]}
/>
...
<PropertiesTable
  content={[
  { name: 'request', type: 'object', description: '传入的 webhook 请求。', properties: [{ type: 'object', parameters: [
