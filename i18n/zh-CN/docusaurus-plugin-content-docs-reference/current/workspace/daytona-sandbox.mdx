...
### 使用自定义镜像和资源 {/* #custom-image-with-resources */}
...
### 文件系统挂载 {/* #filesystem-mounting */}

将 S3 或 GCS 存储桶挂载为 Sandbox 内的本地目录。

#### 通过 Workspace mounts 配置 {/* #via-workspace-mounts-config */}
...
#### 通过 `sandbox.mount()` {/* #via-sandboxmount */}
...
#### S3 兼容服务（Cloudflare R2、MinIO） {/* #s3-compatible-cloudflare-r2-minio */}
...
### S3 环境变量 {/* #s3-environment-variables */}
...
### GCS 环境变量 {/* #gcs-environment-variables */}
...
### 使用快照缩短冷启动延迟 {/* #reducing-cold-start-latency-with-a-snapshot */}
...
## Sandbox 创建模式 {/* #sandbox-creation-modes */}
