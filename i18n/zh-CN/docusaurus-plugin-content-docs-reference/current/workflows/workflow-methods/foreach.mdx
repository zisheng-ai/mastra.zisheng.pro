...
<PropertiesTable
  ...
  },
  {
    name: 'opts',
    type: 'object',
    description: '循环的可选配置。concurrency 选项控制可以并行运行的迭代次数（默认值：1）',
    isOptional: true,
    properties: [
      {
        name: 'concurrency',
...
### 在 `.foreach()` 后使用 `.then()` {/* #using-then-after-foreach */}
...
### 在 `.foreach()` 后使用 `.map()` {/* #using-map-after-foreach */}
...
### 流式传输期间的进度事件 {/* #progress-events-during-streaming */}
...
### 恢复单次迭代 {/* #resuming-a-single-iteration */}
