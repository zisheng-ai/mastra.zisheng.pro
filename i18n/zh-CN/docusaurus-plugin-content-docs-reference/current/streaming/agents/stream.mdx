    ...
    properties: [
      {
        ...
        parameters: [
                        ...
                        parameters: [
                          {
                            ...
                            type: "'none' | 'ratio'",
                            isOptional: false,
                            description: "采样策略的类型。使用 'none' 禁用采样，或使用 'ratio' 进行按比例采样。",
                          },
                        ],
                      },
                      {
                ...
                parameters: [
                  {
                    ...
                    isOptional: true,
                    description: '委派给 Subagent 前调用的回调函数。可用于筛选传递给 Subagent 的消息。',
                  },
                ],
              },
            ],
          ...
          },
          {
            ...
            isOptional: true,
            description: '每个执行步骤完成后调用的回调函数。以 JSON 字符串形式接收步骤详情。结构化输出不支持此选项',
          },
          {
            name: 'telemetry',
          ...
          },
          {
            ...
            type: 'number',
            isOptional: true,
            description: '并发执行的 Tool 调用最大数量。可能需要批准时默认为 1，否则为 10。',
          },
          {
            name: 'providerOptions',
                ...
                parameters: [
                  {
                    ...
                    type: 'Record<string, JSONValue>',
                    isOptional: true,
                    description: '其他 Provider 专用选项。键为 Provider 名称，值为 Provider 专用选项的记录。',
                  },
                ],
              },
            ],
          ...
          },
          {
            ...
            type: 'TracingContext',
            isOptional: true,
            description: '用于创建子 span 和添加元数据的 Tracing 上下文。使用 Mastra 的 Tracing 系统时会自动注入。',
            properties: [
              {
                parameters: [
                  {
                ...
                parameters: [
                  {
                    ...
                    isOptional: true,
                    description: '添加到根 Trace span 的元数据。可用于添加用户 ID、会话 ID 或功能标志等自定义属性。',
                  },
                ],
              },
              {
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    isOptional: true,
    description: '启用 Tracing 时，与本次执行关联的 Trace ID。可用于关联日志和调试执行流程。',
  },
  {
    name: 'spanId',
    type: 'string',
    isOptional: true,
    description: '启用 Tracing 时，与本次执行关联的根 span ID。可用于 span 级查询和关联。',
  },
]}
/>
...
### Mastra 格式（默认） {/* #mastra-format-default */}
...
### 使用选项的高级示例 {/* #advanced-example-with-options */}
