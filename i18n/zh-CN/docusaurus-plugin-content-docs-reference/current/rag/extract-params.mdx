...
## Extractor 参数 {/* #extractor-arguments */}
...
<PropertiesTable
  ...
  },
  {
    ...
    type: "('self' | 'prev' | 'next')[]",
    isOptional: true,
    description: "要生成的摘要类型列表。只能包含 'self'（当前块）、'prev'（前一块）或 'next'（后一块）。",
  },
  {
    name: 'promptTemplate',
    type: 'string',
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    isOptional: true,
    description: '用于问题生成的自定义提示词模板。必须同时包含 {context} 和 {numQuestions} 占位符',
  },
  {
    name: 'embeddingOnly',
    type: 'boolean',
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'string',
    isOptional: true,
    description: '用于关键词提取的自定义提示词模板。必须同时包含 {context} 和 {maxKeywords} 占位符',
  },
]}
/>
...
## 用于标题提取的文档分组 {/* #document-grouping-for-title-extraction */}
