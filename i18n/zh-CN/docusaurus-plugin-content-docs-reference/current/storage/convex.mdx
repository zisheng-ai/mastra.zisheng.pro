...
### 1. 设置 Convex Schema {/* #1-set-up-convex-schema */}
...
### 2. 创建 Storage Handler {/* #2-create-the-storage-handler */}
...
### 3. 部署到 Convex {/* #3-deploy-to-convex */}
...
## ConvexStore 参数 {/* #convexstore-parameters */}
...
## ConvexServerCache 参数 {/* #convexservercache-parameters */}
...
<PropertiesTable
  ...
  },
  {
    ...
    type: 'number',
    description: 'Convex cache mutation 请求的超时时间，单位为毫秒。设置为 0 可禁用 client-side 超时。',
    isOptional: true,
    defaultValue: 30000,
  },
  {
    name: 'keyPrefix',
    type: 'string',
    description: '应用于 ConvexServerCache 键的前缀。`clear()` 会移除已存储前缀与此值完全匹配的行。',
    isOptional: true,
