...
<PropertiesTable
  ...
  },
  {
    name: 'pool',
    type: 'oracledb.Pool',
    description: '现有的 Oracle 连接池。提供后，Mastra 会使用该连接池，但调用 `store.close()` 时不会关闭它。',
    isOptional: true,
  },
  {
    name: 'poolManager',
    type: 'OraclePoolManager',
    description: '共享的 Oracle 连接池管理器。使用它在 `OracleStore` 和 `OracleVector` 之间共享一个 Oracle 连接池。',
    isOptional: true,
  },
  {
    name: 'schemaName',
  ...
  },
  {
    name: 'disableInit',
    type: 'boolean',
    description: '为 true 时，禁用自动 schema 初始化。请在应用启动前通过其他方式单独应用 schema 更改时使用。',
    isOptional: true,
    defaultValue: 'false',
  },
  {
    ...
    type: 'number',
    description: '保存消息时，每次 Oracle `executeMany` 调用发送的消息数量。操作仍会在事务边界处提交一次。',
    isOptional: true,
    defaultValue: '200',
  },
  {
  ...
  },
  {
    ...
    description: '初始化期间要创建的自定义 Oracle 索引定义。索引会路由到拥有目标表的存储域。',
    isOptional: true,
  },
  {
...
### 向 Agent 添加 OracleDB Memory {/* #adding-oracledb-memory-to-an-agent */}
