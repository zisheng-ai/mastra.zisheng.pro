...
### Markdown 格式的 schedule {/* #markdown-schedules */}
...
<PropertiesTable
  ...
  },
  {
    name: 'threadId',
    type: 'string',
    description: '将触发作为 signal 发送到现有 thread，而不是启动新的运行。需要 `resourceId`。',
    isOptional: true,
  },
  {
    name: 'resourceId',
  ...
  },
  {
    ...
    type: 'ScheduleIfIdle',
    description:
      '目标 thread 处于空闲状态时执行的操作：`wake`、`persist` 或 `discard`。仅适用于使用 thread 的 schedule。',
    isOptional: true,
  },
  {
    name: 'status',
...
## 在开发环境中测试 schedule {/* #testing-a-schedule-in-development */}
...
## 注册和生命周期 {/* #registration-and-lifecycle */}
