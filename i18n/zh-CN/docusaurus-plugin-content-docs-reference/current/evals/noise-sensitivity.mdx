    ...
    children: [
      {
        ...
        type: 'string',
        description: '用于比较的预期干净响应（Agent 在无噪声时理想情况下应生成的内容）',
        required: true,
      },
      {
        name: 'noisyQuery',
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            type: 'number',
            description: '当 LLM 分数与计算分数不一致时采用保守评分的阈值（默认为 0.2）',
            required: false,
          },
        ],
      },
...
## CI/测试要求 {/* #citesting-requirements */}

此 Scorer 专为 CI/测试环境设计，并有以下特定要求：

### 为什么这是 CI Scorer {/* #why-this-is-a-ci-scorer */}
...
### 示例：CI 测试实现 {/* #example-ci-test-implementation */}
...
### 5. Hallucination 抵抗能力 {/* #5-hallucination-resistance */}
...
## CI/测试使用模式 {/* #citesting-usage-patterns */}
...
### 质量保证测试 {/* #quality-assurance-testing */}
...
### 完整的 Vitest 示例 {/* #complete-vitest-example */}
...
## 完美稳健性示例 {/* #perfect-robustness-example */}
...
## 易受干扰信息影响的示例 {/* #distractor-vulnerability-example */}
...
## 严重受损示例 {/* #severe-compromise-example */}
...
## 自定义评分配置 {/* #custom-scoring-configuration */}
...
## CI 测试套件：测试不同的噪声类型 {/* #ci-test-suite-testing-different-noise-types */}
...
## CI pipeline：用于模型比较的批量评估 {/* #ci-pipeline-batch-evaluation-for-model-comparison */}
...
## CI 中的安全测试 {/* #security-testing-in-ci */}
