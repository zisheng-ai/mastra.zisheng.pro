...
## Trajectory 提取 {/* #trajectory-extraction */}

`runEvals` pipeline 会根据是否配置可观测性 storage，使用以下两种提取策略：

### 基于 trace 的提取（首选） {/* #trace-based-extraction-preferred */}
...
## Trajectory 类型 {/* #trajectory-types */}
...
## 选择 Scorer {/* #choosing-between-scorers */}

### 以下情况使用基于代码的 Scorer： {/* #use-the-code-based-scorer-when */}
...
### 以下情况使用基于 LLM 的 Scorer： {/* #use-the-llm-based-scorer-when */}
...
## 基于代码的 Trajectory Accuracy Scorer {/* #code-based-trajectory-accuracy-scorer */}
...
### 预期 trajectory 的来源 {/* #expected-trajectory-sources */}
...
#### 严格模式（`strictOrder: true`） {/* #strict-mode-strictorder-true */}

要求完全匹配。实际步骤必须按相同顺序匹配预期步骤，不能有额外或缺失步骤。完全匹配时返回 `1.0`，否则返回 `0.0`。

#### 宽松模式（`strictOrder: false`，默认） {/* #relaxed-mode-strictorder-false-default */}

允许存在额外步骤。预期步骤必须以正确的相对顺序出现。分数根据匹配的预期步骤数量计算，还可选择对额外或重复步骤扣分。

## 基于代码的评分详情 {/* #code-based-scoring-details */}

- **连续分数**：宽松模式返回 0.0 到 1.0 之间的值；严格模式返回二元值（0 或 1）
- **确定性**：相同输入始终产生相同输出
- **快速**：无需外部 API 调用

### 基于代码的 Scorer 结果 {/* #code-based-scorer-results */}
...
## 基于代码的 Scorer 示例 {/* #code-based-scorer-examples */}

### 采用严格顺序的 Agent trajectory {/* #agent-trajectory-with-strict-ordering */}
...
### 采用宽松顺序的 Agent trajectory {/* #agent-trajectory-with-relaxed-ordering */}
...
## 基于 LLM 的 Trajectory Accuracy Scorer {/* #llm-based-trajectory-accuracy-scorer */}
...
## 基于 LLM 的评分详情 {/* #llm-based-scoring-details */}
...
### 基于 LLM 的 Scorer 选项 {/* #llm-based-scorer-options */}
...
### 基于 LLM 的 Scorer 结果 {/* #llm-based-scorer-results */}
...
## 统一 Trajectory Scorer {/* #unified-trajectory-scorer */}
...
### 统一 Scorer 结果 {/* #unified-scorer-results */}
...
### 每个数据项的预期 {/* #per-item-expectations */}
...
### 示例：效率和黑名单 {/* #example-efficiency-and-blacklist */}
...
## 将 Trajectory Scorer 与 `runEvals` 搭配使用 {/* #using-trajectory-scorers-with-runevals */}
