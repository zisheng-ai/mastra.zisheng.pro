    ...
    properties: [
      {
        ...
        parameters: [
          ...
          },
          {
            ...
            description: '从运行输入和输出中提取源文本的函数。优先级高于 source',
            isOptional: true,
          },
          {
            name: 'maxQuestions',
...
<PropertiesTable
  content={[
  {
    name: 'score',
    type: 'number',
    description: '介于 0 和 scale 之间的摘要分数（默认为 0-1），取 Alignment 与 Coverage 分数中的较低值',
  },
  {
    name: 'reason',
    type: 'string',
    description: '易于理解的说明，指出决定最终分数的维度及其相关声明或问题。文本中会显示两个维度的分数',
  },
  {
...
### 分数未涵盖的内容 {/* #what-the-score-leaves-out */}
...
### 对运行输入进行摘要 {/* #summarizing-the-run-input */}
...
### 对其他位置的文档进行摘要 {/* #summarizing-a-document-from-elsewhere */}
...
## 与 Faithfulness 的比较 {/* #comparison-with-faithfulness */}
