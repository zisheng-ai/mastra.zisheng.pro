...
你會在 90 分鐘內使用 Mastra，以 TypeScript 建立並部署第一個 Agent。在過程中，你將了解 Agent 迴圈的底層運作方式、Tool 如何讓 Agent 存取其他系統、MCP 如何開啟外部資源的大門，以及情境工程與記憶如何在對話過程中形塑行為。
  ...
  lessons: [
    // Module 1：開始使用
    {
      ...
      preview: {
        ...
        bullets: [
          'Agent 與聊天機器人的差異',
          'Mastra 的核心元件：Agent、Tool、Workflow、記憶、檢索與可觀測性',
    ...
    },
    {
      ...
      preview: {
        ...
        bullets: [
          ...
          '在本機建立並執行 Mastra 專案',
          '瀏覽 Studio：Agents、Workflows、Tools 與 Traces',
        ],
      },
      seo: {
    ...
    },
    {
      ...
      preview: {
        intro:
          '將 Studio 中看到的內容，對應到 create-mastra 建立的專案配置。以專案骨架中的 Weather Agent、Workflow 與 Tool 作為參考。',
        bullets: [
    ...
    // Module 2：Tools
    {
      ...
      preview: {
        intro:
          '建立一個簡單的 Tool、在 Studio 中單獨測試、將它連接至 Agent，並提示 Agent 呼叫這個 Tool。接著開啟 Traces，查看 Tool 呼叫及其結果。',
        bullets: [
    ...
    },
    {
      ...
      preview: {
        ...
        bullets: [
          'Agents：開放式目標，由模型決定步驟與停止時機',
          'Workflows：預先定義步驟，由你控制路徑與停止條件',
    ...
    },
    {
      ...
      preview: {
        ...
        bullets: [
          ...
          'Traces 端對端呈現 Workflow 步驟、Tool 呼叫與輸出',
        ],
      },
    },
    ...
    // Module 4：記憶
    {
      ...
      preview: {
        intro:
          '模型在不同呼叫之間不保留狀態，而記憶能讓後續對話延續下去。在 Studio 中追蹤一段真實對話，查看 Agent 實際收到哪些情境，並了解 lastMessages 設定如何控制每次呼叫納入多少歷史記錄。',
        bullets: [
    ...
    },
    {
      ...
      preview: {
        intro:
          '原始訊息歷史記錄很快就會大量累積。啟用 Observational Memory，將較舊的情境自動壓縮成資訊密度更高的觀察結果，再測試以資源為範圍的記憶，讓同一位使用者的偏好能跨討論串延續。',
        bullets: [
          'OM 透過 Observer 與 Reflector 背景 Agents 壓縮較舊的歷史記錄',
          '資源範圍：記憶會隨使用者跨討論串延續，而非只存在於單一對話中',
    ...
    },
    {
      ...
      preview: {
        intro:
          '將已部署的主題樂園 Agent 連線至 Slack，讓你能從任何地方傳送訊息給它，並在私訊中使用相同的 Tools、記憶與 Workflows。',
        bullets: [
